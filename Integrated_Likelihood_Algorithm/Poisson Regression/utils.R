...
likelihood <- function(Beta, X_design, Y, inv_link = exp) {
  exp(log_likelihood(Beta, X_design, Y, inv_link))
}
...
get_omega_hat <- function(
  ...
  if (
    abs(omega_hat_eq_con_fn(omega_hat)) <= 0.1 &&
      omega_hat_ineq_con_fn(omega_hat) <= 0
  ) {
  ...
  result <- foreach(
  ...
  ) %dofuture%
    {
      ...
      for (i in seq_along(psi_grid)) {
        ...
        log_L_tilde_vec[i] <- log_likelihood_rcpp(
          Beta_hat,
          X_design,
          Y_design,
          Jm1,
...
get_log_integrated_likelihood <- function(
  ...
  return(list(branches = branches, log_L_bar = log_L_bar))
}
