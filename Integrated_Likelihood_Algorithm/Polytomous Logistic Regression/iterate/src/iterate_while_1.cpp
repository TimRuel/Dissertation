...
NumericMatrix iterate_while_1(
  ...
  while (current_branch_val_forward > stopping_branch_val && current_psi_val_forward <= max_psi_val) {
    // Store previous log-likelihood before updating
    double previous_loglikelihood = current_branch_val_forward;
    
    NumericVector Beta_hat = as<NumericVector>(
      get_Beta_hat(current_guess, current_psi_val_forward, omega_hat, X_one_hot, X_h_one_hot, shift)
    );
    current_branch_val_forward = as<double>(
      get_log_likelihood(Beta_hat, X_one_hot, Y_one_hot)
    );
    
    temp = previous_loglikelihood - current_branch_val_forward;
  ...
  while (current_branch_val_backward > stopping_branch_val && current_psi_val_backward >= 0) {
    // Store previous log-likelihood before updating
    double previous_loglikelihood = current_branch_val_backward;
    
    NumericVector Beta_hat = as<NumericVector>(
      get_Beta_hat(current_guess, current_psi_val_backward, omega_hat, X_one_hot, X_h_one_hot, shift)
    );
    current_branch_val_backward = as<double>(
      get_log_likelihood(Beta_hat, X_one_hot, Y_one_hot)
    );
    
    temp = previous_loglikelihood - current_branch_val_backward;
