...
get_CI_logit_mean_response <- function(x, y, x_h, alpha) {
  ...
  SE_estimated_logit_mean_response <- get_SE_estimated_logit_mean_response(
    x,
    y,
    x_h
  )
...
get_omega_hat <- function(u, beta_MLE, x_h) {
  u * get_linear_predictor(beta_MLE, x_h) / get_linear_predictor(u, x_h)
}

get_omega_hat_list <- function(u_list, beta_MLE, x_h) {
  purrr::map(u_list, \(u) get_omega_hat(u, beta_MLE, x_h))
}
...
get_beta_hat <- function(init_guess, psi, omega_hat, x, x_h) {
  ...
  f <- function(beta) {
    ...
    -sum(
      logistic_mean_response * linear_predictor - log(1 + exp(linear_predictor))
    )
  }
...
get_log_L_tilde <- function(
  ...
  beta_hat_method |>
    ...
    switch(
      accumulate = accumulate_beta_hats(
        psi_grid_list,
        omega_hat,
        x,
        x_h,
