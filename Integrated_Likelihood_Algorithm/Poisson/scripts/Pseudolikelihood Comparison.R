...
conf_ints <- pseudo_log_likelihood_curves |>
  map2(MLE_data$MLE, function(curve, MLE) {
    lower_bound <- tryCatch(
      uniroot(function(psi) curve(psi) + crit, interval = c(min_psi, MLE))$root,
    ...
    upper_bound <- tryCatch(
      uniroot(function(psi) curve(psi) + crit, interval = c(MLE, max_psi))$root,
  ...
  map2(
    ...
    function(curve, pseudolikelihood_name) {
      stat_fn <- stat_function(
        fun = curve,
        geom = "line",
        aes(color = pseudolikelihood_name, linewidth = pseudolikelihood_name),
        show.legend = FALSE
      )
      return(stat_fn)
    }
...
base_plot +
  ...
  geom_text(aes(
    x_range[2] - 0.25 * diff(x_range),
    -crit,
    label = "95% CI Line",
    vjust = 1.3
  )) +
  geom_vline(
    aes(xintercept = MLE, color = Pseudolikelihood),
    data = label_data,
    show.legend = FALSE
  ) +
  ggrepel::geom_label_repel(
    aes(x = MLE, y = y_min / 2, label = MLE_label, color = Pseudolikelihood),
    data = label_data,
    direction = "y",
    parse = TRUE,
    show.legend = FALSE
