...
inject_shared_args <- function(cfg, shared_name = "shared_args") {
  ...
  cfg[names(cfg) != shared_name] <- lapply(
    cfg[names(cfg) != shared_name],
    inject
  )
...
load_config <- function(population) {
  ...
  pop_result_base <- file.path(
    cfg$output_dir,
    sprintf("population_%s", population)
  )
  dir.create(pop_result_base, recursive = TRUE, showWarnings = FALSE)

  existing_runs <- list.dirs(
    pop_result_base,
    full.names = FALSE,
    recursive = FALSE
  )
...
get_theta_hat <- function(lambda, omega_hat, weights) {
  omega_hat / (1 - lambda * weights)
}
...
get_branch_df <- function(omega_hat, weights, lambda_grid) {
  ...
  branch_df <- data.frame(psi = psi_grid, Integrated = log_L_tilde)
...
generate_branches <- function(cfg, envir = environment()) {
  ...
  result <- foreach::foreach(
    ...
    .options.future = list(seed = TRUE, chunk.size = chunk_size)
  ) %dofuture%
    {
      ...
      list(omega_hat = omega_hat, branch_df = branch_df)
    }
  ...
  list(omega_hat = omega_hat, branch_dfs = branch_dfs)
}
...
get_log_L_bar <- function(branches, psi_grid) {
  ...
  log_L_bar_df <- data.frame(psi = psi_grid, Integrated = log_L_bar)
...
find_MLE <- function(PLL_df_long, model) {
  ...
  optimize(objective, lower = lower, upper = upper, maximum = TRUE)
}
...
get_MLE_data <- function(PLL_df_merged_long, spline_fitted_models) {
  spline_fitted_models |>
    ...
    dplyr::rename(MLE = maximum, Maximum = objective) |>
...
find_PLL_CI_endpoints <- function(PLLs, PLL_df_long, alpha) {
  ...
  search_intervals <- purrr::map2(PLLs, PLL_types, \(PLL, type) {
    PLL_df_long |>
  ...
  endpoints <- purrr::map2(PLLs, search_intervals, \(PLL, interval) {})
