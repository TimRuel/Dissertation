...
ggplot() +
  ...
  theme_minimal() +
  theme(legend.position = c(0.8, 0.8), legend.background = element_rect())
...
u_list |>
  map_dbl(\(u) {
    sum(log(do.call(density, args = c(x = list(u), nominal_density_params))))
  })
...
data.frame(
  ...
  ggplot() +
  geom_point(aes(x = psi, y = Log_Variance, color = Method), size = 0.1) +
  ylab("Log Variance") +
...
get_L_bar <
  function(psi, data, weights, MC_params, R) {
    psi_hat <- dot_product(data, weights)
