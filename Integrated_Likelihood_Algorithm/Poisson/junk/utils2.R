        ...
        w <- u_list |>
          map_dbl(\(u) {
            p <- MC_params$nominal$density |>
              do.call(
                args = c(x = list(u), MC_params$nominal$density_params)
              ) |>
              prod()

            q <- MC_params$importance$density |>
              do.call(
                args = c(x = list(u), MC_params$importance$density_params)
              ) |>
              prod()
...
get_omega_hat_list <- function(u_list, psi_hat, weights) {
  purrr::map(u_list, \(u) Q(u, psi_hat, weights))
}
...
get_L_tilde <- function(
  ...
  lambda_method |>
    ...
    switch(
      accumulate = get_lambdas_accumulate(
        psi_grid,
        omega_hat,
        weights,
        init_lambda
  ...
  MC_params$method |>
    ...
    switch(
      ...
      regression_IS = {
        ...
        weighted_vals <- sweep(L_tilde_mat, 1, w, '*') -
          outer(w - 1, beta_hat, '*')
      }
    )
...
get_integrated_likelihood <- function(
  data,
  weights,
  psi_grid,
  R,
...
get_importance_sampling_diagnostics <- function(integrated_log_likelihood) {
  ...
  gamma_dens <- ddply(density_args, "count", function(df) {
    data.frame(
      ...
      importance = dgamma(
        grid,
        mean(df$importance_shape),
        mean(df$importance_rate)
      )
    )
  }) |>
    tidyr::pivot_longer(
      cols = c("nominal", "importance"),
      values_to = "density"
    )

  density_plots <- gamma_dens |>
    ggplot() +
    geom_line(aes(x = x, y = density, color = name)) +
    facet_wrap(~count) +
  ...
  variance_plot <- data.frame(
    psi = integrated_log_likelihood$psi_grid,
    variance = integrated_log_likelihood$L_bar$var_estimate
  ) |>
    ggplot() +
    geom_point(aes(x = psi, y = variance), size = 0.1) +
    ylab("Variance") +
    xlab(expression(psi)) +
    theme_minimal() +
...
get_profile_log_likelihood <- function(
  data,
  weights,
  step_size,
  num_std_errors
) {
  psi_grid_list <- get_psi_grid(
    data,
    weights,
    step_size,
    num_std_errors,
