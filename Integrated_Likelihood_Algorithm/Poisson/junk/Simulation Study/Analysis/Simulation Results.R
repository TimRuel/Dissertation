...
integrated_log_likelihood_sims <- readRDS(
  integrated_log_likelihood_sims_file_path
)

mod_integrated_log_likelihood_sims_file_path <- file.choose()

mod_integrated_log_likelihood_sims <- readRDS(
  mod_integrated_log_likelihood_sims_file_path
)
...
population <- gsub(
  ".*/Simulations/(.*)/Integrated Likelihood/.*",
  "\\1",
  integrated_log_likelihood_sims_file_path
)
...
multinomial_entropy_sims_lists <- list(
  "Integrated" = integrated_log_likelihood_sims,
  "Mod_Integrated" = mod_integrated_log_likelihood_sims,
  "Profile" = profile_log_likelihood_sims
)

psi_grid_lists <- list(
  "Integrated" = psi_grid_list_IL,
  "Mod_Integrated" = psi_grid_list_mod_IL,
  "Profile" = psi_grid_list_PL
)

spline_fitted_models_list <- multinomial_entropy_sims_lists |>
  map2(psi_grid_lists, \(sims_list, psi_grid_list) {
    sims_list |>
      map2(psi_grid_list, \(sims, psi_grid) {
        data.frame(psi = psi_grid, loglikelihood = sims) |>
          (\(df) smooth.spline(df$psi, df$loglikelihood))()
      })
  })

MLE_data_list <- spline_fitted_models_list |>
  map2(psi_grid_lists, \(spline_fitted_models, psi_grid_list) {
    spline_fitted_models |>
      map2(psi_grid_list, \(mod, psi_grid) {
        optimize(
          \(psi) predict(mod, psi)$y,
          lower = psi_grid |> head(1),
          upper = psi_grid |> tail(1),
          maximum = TRUE
        ) |>
          data.frame() |>
          mutate(MLE = as.numeric(maximum), Maximum = as.numeric(objective)) |>
          select(MLE, Maximum)
      }) |>
      bind_rows()
  })
...
pseudo_log_likelihood_curves_list <- spline_fitted_models_list |>
  map2(MLE_data_list, function(spline_fitted_models, MLE_data) {
    spline_fitted_models |>
      map2(MLE_data$Maximum, function(mod, maximum) {
        function(psi) predict(mod, psi)$y - maximum
      })
  })
...
conf_ints_list <- pseudo_log_likelihood_curves_list |>
  map2(MLE_data_list, function(pseudo_log_likelihood_curves, MLE_data) {
    pseudo_log_likelihood_curves |>
      map2(MLE_data$MLE, function(curve, MLE) {
        lower_bound <- tryCatch(
          uniroot(function(psi) curve(psi) + crit, interval = c(0, MLE))$root,
        ...
        upper_bound <- tryCatch(
          uniroot(
            function(psi) curve(psi) + crit,
            interval = c(MLE, log(length(data)))
          )$root,
...
multinomial_entropy_sim_results <- MLE_data_list |>
  map2(conf_ints_list, function(MLE_data, conf_ints) {
    MLE_data |>
      mutate(
...
saveRDS(
  multinomial_entropy_sim_results,
  paste0("Results/", multinomial_entropy_sim_results_file_path)
)
