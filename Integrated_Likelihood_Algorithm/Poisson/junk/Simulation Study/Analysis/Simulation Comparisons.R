...
desert_rodents_sim_results <- readRDS(
  "Results/desert_rodents_sim_results.Rda"
) |>
  dplyr::rename(
    "Desert Rodents (Integrated)" = "Integrated",
    "Desert Rodents (Mod_Integrated)" = "Mod_Integrated",
    "Desert Rodents (Profile)" = "Profile"
  )

birds_in_balrath_woods_sim_results <- readRDS(
  "Results/birds_in_balrath_woods_sim_results.Rda"
) |>
  dplyr::rename(
    "Birds in Balrath Woods (Integrated)" = "Integrated",
    "Birds in Balrath Woods (Mod_Integrated)" = "Mod_Integrated",
    "Birds in Balrath Woods (Profile)" = "Profile"
  )

birds_in_killarney_woodlands_sim_results <- readRDS(
  "Results/birds_in_killarney_woodlands_sim_results.Rda"
) |>
  dplyr::rename(
    "Birds in Killarney Woodlands (Integrated)" = "Integrated",
    "Birds in Killarney Woodlands (Mod_Integrated)" = "Mod_Integrated",
    "Birds in Killarney Woodlands (Profile)" = "Profile"
  )

ruel_sim_results_df <- list(
  desert_rodents_sim_results,
  birds_in_balrath_woods_sim_results,
  birds_in_killarney_woodlands_sim_results
) |>
  reduce(plyr::join, by = "Metric") |>
  add_column(Author = "Ruel", .after = "Metric")

severini_sim_results_df <- data.frame(
  c("Bias", "SD", "RMSE", "Coverage", "Length"),
  ...
  add_column(Author = "Severini", .after = "Metric")
...
sim_results_df1 |>
  ...
  footnote(
    general = "Each simulation was based on 1000 Monte Carlo replications.
                      The nominal coverage probability of the confidence intervals was 95%.",
    general_title = "",
...
sim_results_df3 <- sim_results_df1 |>
  ...
  mutate(
    Likelihood = ifelse(
      grepl("Integrated", name),
      ifelse(grepl("Mod_Integrated", name), "Mod_Integrated", "Integrated"),
      "Profile"
    ),
...
sim_results_df3 |>
  kbl(
    col.names = c("Metric", rep(c("Ruel", "Severini"), 3)),
    align = "c",
    digits = 3,
    row.names = FALSE,
    escape = FALSE
  ...
  column_spec(
    c(3, 5, 7),
    color = c(rep("black", 5), rep("grey", 5), rep("black", 5))
  ) |>
  # column_spec(c(4, 6), color = c(rep("red", 5), rep("grey", 5), rep("black", 5))) |>
  pack_rows(
    index = c("Integrated" = 5, "Modified Integrated" = 5, "Profile" = 5),
    label_row_css = "background-color: #666; color: #fff;"
  ) |>
  footnote(
    general = "Each simulation was based on 1000 Monte Carlo replications.
