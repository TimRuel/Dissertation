...
get_theta_hat <- function(lambda, m, omega_hat, weights) {
  (m * omega_hat) / (m + lambda * weights)
}
...
custom_combine_IL <- function(...) {
  ...
  preallocations <- matrix(
    data = NA,
    nrow = length(arglist),
    ncol = length(data)
  )

  for (result in arglist) {
    preallocations[result$i, ] <- result$preallocations
  }

  return(list(data = data, preallocations = preallocations))
}

get_IL_preallocations <- function(
  data_sims,
  weights,
  u_params,
  R,
...
get_integrated_log_likelihood_vals <- function(
  data,
  weights,
  step_size,
  num_std_errors,
...
get_integrated_log_likelihood_sims <- function(
  preallocations,
  weights,
  step_size,
  num_std_errors,
  ...
  foreach(
    ...
    ) %dofuture%
    {
      ...
      get_integrated_log_likelihood_vals.aux(
        omega_hat,
        data,
        weights,
        psi_grid_list
...
get_mod_IL_preallocations <- function(
  data_sims,
  weights,
  Q,
  prior,
  ...
  mod_IL_preallocations <-
    ...
    ) %dofuture%
    {
      ...
      u_params <- list(alpha = data + prior$alpha, beta = 1 + prior$beta)
...
get_mod_integrated_log_likelihood_vals <- function(
  data,
  weights,
  step_size,
  num_std_errors,
...
get_mod_integrated_log_likelihood_sims <- function(
  preallocations,
  weights,
  step_size,
  num_std_errors,
