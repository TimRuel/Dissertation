...
plot_entropy <- function(X1_levels, Y_probs, title) {
  ...
  plot_base() +
    geom_line(aes(x = X2, y = entropy, color = X1), linewidth = 1, data = df) +
    geom_hline(
...
get_theoretical_entropy_plot <- function(X1_levels, pY_0) {
  ...
  entropy_plot <- plot_entropy(X1_levels, pY_0, title)
...
get_observed_plots <- function(X1_levels, Y_probs, df) {
  ...
  observed_entropy_plot <- plot_entropy(X1_levels, Y_probs, title)
