...
base_path <- if (is.null(sim_id)) "individual_runs" else file.path("simulations", sim_id)
run_dir <- proj_path("experiments", experiment_id, base_path, run_id)
...
subtitle <- paste(
  ...
  if (!is.null(sim_id)) paste("— Simulation", sim_id, "— Iteration", run_id)
  else paste("— Individual Run", run_id)
)
