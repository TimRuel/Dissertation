...
experiment_id <- if (length(args) > 0) {
  args[1]
} else {
  stop("[ERROR] Provide experiment ID")
}
...
write_strict_yaml(experiment_config, dest_path)
message(
  "[INFO] Created and saved config to /",
  sub(".*(/?config/.*)", "\\1", dest_path)
)
