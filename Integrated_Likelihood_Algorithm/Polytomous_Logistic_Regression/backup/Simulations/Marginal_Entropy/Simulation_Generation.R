...
setwd(
  "/gpfs/home/tbr0780/Dissertation/Integrated_Likelihood_Algorithm/Polytomous Logistic Regression/Simulations/Marginal Entropy"
)
source("../../utils.R")
seed <- 7835
set.seed(seed)
...
for (h in X1_levels) {
  ...
  log_integrated_likelihood_filepath <- glue::glue(
    "IL_objects/h={h}/Sim{TASK_ID}.Rda"
  )
  ...
  log_profile_likelihood_filepath <- glue::glue(
    "PL_objects/h={h}/Sim{TASK_ID}.Rda"
  )
  ...
  log_likelihood_df_filepath <- glue::glue(
    "log_likelihood_dfs/h={h}/Sim{TASK_ID}.Rda"
  )
