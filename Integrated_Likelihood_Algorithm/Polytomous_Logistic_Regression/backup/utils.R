...
optimize_Beta_0 <- function(J, X2_interval, H_min, H_max, num_vals, Beta2) {
  ...
  result <- auglag(
    x0 = init_params,
    fn = function(params) {
      Beta_0_objective_fn(params, X2_vals, H_min, H_max, Beta2)
    },
    lower = rep(-10, length(init_params)),
    upper = rep(10, length(init_params)),
    hin = function(params) {
      Beta_0_constraint_fn(params, X2_vals, H_min, H_max, Beta2)
    },
...
get_Beta_0 <- function(X1_levels, p, J, X2_intervals, num_vals) {
  ...
  Beta_0 <- matrix(NA, nrow = p, ncol = J - 1)
  ...
  for (h in X1_levels) {
    ...
    if (h == X1_ref_level) {
      ...
      Beta_0[grepl(h, rownames(Beta_0)), ] <- unname(rbind(
        params$intercept,
        params$slope
      ))
    }
...
E_log_like <- function(omega, X_design, Beta) {
  ...
  probs <- X_design %*% omega |> apply(1, softmax_adj) |> t()
...
get_Beta_hat <- function(
  ...
  for (attempt in 1:10) {
    result <- safe_auglag(
      ...
      localtol = 1e-3,
      deprecatedBehavior = FALSE,
      control = list(on.error = "ignore", maxtime = maxtime)
    )
  ...
  result_fallback <- safe_auglag(
    ...
    localtol = 1e-3,
    deprecatedBehavior = FALSE,
    control = list(on.error = "ignore", maxtime = maxtime + 10)
  )
...
get_omega_hat <- function(
  ...
  if (
    !is.null(result$par) &&
      abs(omega_hat_eq_con_fn(result$par)) <= 0.1 &&
      omega_hat_ineq_con_fn(result$par) <= 0
  ) {
    omega_hat <- matrix(result$par, nrow = p, ncol = Jm1, byrow = FALSE)

    return(omega_hat)
  } else {
    return(get_omega_hat(
      omega_hat_eq_con_fn,
      omega_hat_ineq_con_fn,
      Jm1,
      p,
...
make_omega_hat_branch_fn <- function(
  omega_hat,
  X_design,
  Y_design,
  X_h_design,
  ...
  function(psi) {
    ...
    Beta_hat <- get_Beta_hat(obj_fn, con_fn, c(omega_hat), NULL, maxtime)
...
generate_branches <- function(
  ...
  result <- foreach(
  ...
  ) %dofuture%
    {
      ...
      log_L_tilde_df <- data.frame(psi = psi_grid, Integrated = log_L_tilde_vec)

      list(omega_hat = omega_hat, log_L_tilde_df = log_L_tilde_df)
    }
...
get_log_integrated_likelihood <- function(
  data,
  formula,
  h,
  step_size,
...
get_log_profile_likelihood <- function(
  data,
  formula,
  h,
  step_size,
  ...
  for (i in seq_along(psi_grid)) {
    ...
    log_L_p_vec[i] <- log_likelihood_rcpp(
      Beta_hat,
      X_design,
      Y_design,
      Jm1,
  ...
  log_L_p_df <- data.frame(psi = psi_grid, Profile = log_L_p_vec)
