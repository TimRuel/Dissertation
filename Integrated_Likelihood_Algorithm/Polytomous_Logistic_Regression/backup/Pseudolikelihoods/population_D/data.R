...
X2_intervals <- list("A" = c(0, 1), "B" = c(0, 1), "C" = c(0, 1))

X2_shape_params <- list("A" = c(2, 5), "B" = c(3, 3), "C" = c(5, 2))
...
true_probs <- X_design %*%
  ...
  mutate(X1 = rep(X1_levels, each = n_samples), X2 = X2) |>
...
base_plot +
  geom_line(aes(x = X2, y = entropy, color = X1), size = 1, data = entropy_df) +
  geom_hline(
    yintercept = unlist(entropy_ranges),
    color = "black",
    size = 0.6,
    linetype = "dashed"
  ) +
  labs(
    title = "True Entropy of Probabilities of Y Classes by Theoretical X1 and X2",
    x = "X2",
    y = "Entropy",
    color = "X1 Level"
  )
...
Y_probs <- X_design %*%
  ...
  data.frame() |>
  rename_with(~ paste0("Y", 1:J)) |>
  mutate(X1 = rep(X1_levels, times = m), X2 = X2) |>
  select(X1, X2, everything())
...
base_plot +
  geom_point(
    aes(x = X2, y = Probability, color = Y_class),
    size = 1,
    data = Y_probs_long
  ) +
  facet_wrap(
  ...
  labs(
    title = "True Probabilities of Y Classes by X1 and X2",
    x = "X2",
    y = "Probability",
    color = "Class of Y"
  )
...
base_plot +
  ...
  labs(
    title = "True Entropy of Probabilities of Y Classes by Observed X1 and X2",
    x = "X2",
    y = "Entropy",
    color = "X1 Level"
  )
...
base_plot +
  ...
  labs(
    title = "Observed Y Values by X1 and X2",
    x = "X2",
    y = "Count",
    fill = "Observed Y"
  )
