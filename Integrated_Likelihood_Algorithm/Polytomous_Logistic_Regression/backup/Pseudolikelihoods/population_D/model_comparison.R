...
for (h in X1_levels) {
  ...
  entropy_errors_interaction <- entropy_rmse(
    model_interaction,
    test_data_h,
    true_entropy
  )
  entropy_errors_main_effects <- entropy_rmse(
    model_main_effects,
    test_data_h,
    true_entropy
  )
  entropy_errors_reduced <- entropy_rmse(
    model_reduced,
    test_data_h,
    true_entropy
  )
...
p_value_lrt <- pchisq(
  lrt_stat,
  df = length(coef(model_interaction)) - length(coef(model_main_effects)),
  lower.tail = FALSE
)
...
p_value_lrt <- pchisq(
  lrt_stat,
  df = length(coef(model_main_effects)) - length(coef(model_reduced)),
  lower.tail = FALSE
)
