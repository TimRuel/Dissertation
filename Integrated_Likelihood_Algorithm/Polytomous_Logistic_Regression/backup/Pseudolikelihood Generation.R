...
threshold <- ceiling(abs(log_likelihood(
  Beta_MLE,
  X_design,
  model.matrix(~Y)[, -1]
))) +
...
log_integrated_likelihood_filepath <- glue::glue(
  "IL_objects/R={num_branches}_J={J}_h={h}_stepsize={step_size}_numse={num_std_errors}.Rda"
)
...
log_profile_likelihood_filepath <- glue::glue(
  "PL_objects/R={num_branches}_J={J}_h={h}_stepsize={step_size}_numse={num_std_errors}.Rda"
)
...
log_integrated_likelihood <- readRDS(
  "Integrated_Likelihood_Algorithm/Polytomous Logistic Regression/Simulations/Marginal Entropy/IL_objects/h=A/Sim1.Rda"
)
...
log_likelihood_vals_file_path <- glue::glue(
  "log_likelihood_vals/R={num_branches}_J={J}_h={h}_stepsize={step_size}_numse={num_std_errors}.Rda"
)

saveRDS(log_likelihood_vals, log_likelihood_vals_file_path)


for (df in log_integrated_likelihood$log_L_tilde_df) {
  plot(df)
}
