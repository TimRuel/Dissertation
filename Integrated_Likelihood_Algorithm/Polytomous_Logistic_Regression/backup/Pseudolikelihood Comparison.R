...
log_likelihood_vals <- readRDS(log_likelihood_vals_file_path) |>
  tidyr::pivot_longer(
    cols = all_of(pseudolikelihood_names),
...
base_plot +
  geom_point(
    aes(x = psi, y = loglikelihood, color = Pseudolikelihood),
  ...
  map2(MLE_data$MLE, function(curve, MLE) {
    lower_bound <- tryCatch(
      uniroot(
        ...
        interval = c(psi_grid |> head(1), MLE)
      )$root,
    ...
    upper_bound <- tryCatch(
      uniroot(
        function(psi) curve(psi) + crit,
        interval = c(MLE, psi_grid |> tail(1))
      )$root,
...
MLE_data |>
  ...
  arrange(length) |>
  add_row(Source = "Truth", MLE = psi_0) |>
  kbl(
    col.names = c("Source", "MLE", "95% Confidence Interval", "CI Length"),
    align = "c"
  ) |>
  kable_styling(bootstrap_options = c("striped", "hover")) |>
  row_spec(3, color = "green", bold = TRUE)

c(stat_fn_IL, stat_fn_PL) %<-%
  map2(
    pseudo_log_likelihood_curves,
    pseudolikelihood_names,
    function(curve, pseudolikelihood_name) {
      stat_fn <- stat_function(
        fun = curve,
        geom = "line",
...
psi_range <- c(
  min(log_likelihood_vals$psi) - step_size,
  max(conf_ints$psi[2], conf_ints$Integrated[2]) + step_size
)

y_min <- pseudo_log_likelihood_curves[[1]](conf_ints$Integrated[1]) - 0.5

label_data <- MLE_data |>
  add_row(Source = "Truth", MLE = psi_0, MLE_label = "psi[0]")

base_plot +
  stat_fn_IL +
  stat_fn_PL +
  geom_hline(yintercept = 0, linetype = 5) +
  geom_hline(aes(yintercept = -crit), linewidth = 1.5) +
  ...
  geom_point(
    aes(x = psi, y = loglikelihood - max(loglikelihood, na.rm = TRUE)),
    data = log_likelihood_vals |>
  ...
  ylab("Log-Likelihood") +
  scale_x_continuous(expand = c(0, 0), limits = psi_range) +
