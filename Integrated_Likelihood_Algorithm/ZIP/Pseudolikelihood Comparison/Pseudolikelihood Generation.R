...
L_bar <- phi_vals |>
  ...
  unlist() |>
  matrix(ncol = length(mu_grid), byrow = TRUE) |>
  colMeans()
...
l_p <- function(mu) {
  n *
    (Y_bar *
      log(mu) -
      (1 - pi_bar) * (log(1 - exp(-mu)) + mu) +
      pi_bar * log(pi_bar) +
      (1 - pi_bar) * log(1 - pi_bar))
}
...
x1 <- (phi_vals[[1]] + (1 - phi_vals[[1]]) * exp(-mu_hat))^(n * pi_bar) *
  (1 - phi_vals[[1]])^(n * (1 - pi_bar))
x2 <- (phi_vals[[2]] + (1 - phi_vals[[2]]) * exp(-mu_hat))^(n * pi_bar) *
  (1 - phi_vals[[2]])^(n * (1 - pi_bar))
...
log_likelihood_vals_file_path <- population_params_file_path |>
  ...
  glue::glue(
    "R={R}_seed={seed}_stepsize={step_size}_numse={num_std_errors}.Rda"
  )
