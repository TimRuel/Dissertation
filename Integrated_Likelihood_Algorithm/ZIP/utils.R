...
log_likelihood <- function(mu, rho, y) {
  ...
  n *
    (rho_bar *
      log(rho + (1 - rho) * exp(-mu)) +
      (1 - rho_bar) * (log(1 - rho) - mu) +
      y_bar * log(mu))
}
...
likelihood <- function(mu, rho, y) {
  ...
  (((rho + (1 - rho) * exp(-mu))^rho_bar) *
    (((1 - rho) * exp(-mu))^(1 - rho_bar)) *
    (mu^y_bar))^n
}

neg_log_likelihood <- function(mu, rho, y) -log_likelihood(mu, rho, y)

get_rho <- function(mu, phi, mu_hat) {
  (phi + (1 - phi) * exp(-mu_hat) - exp(-mu)) / (1 - exp(-mu))
}
