...
psi_MLE_SE <- sqrt(
  sum(matrix(1 + log(theta_MLE)) %*% (1 + log(theta_MLE)) * sigma) / n
)
...
euclidean_distance <- function(u, omega) {
  dist(matrix(c(u, omega), nrow = 2, byrow = TRUE), method = "euclid")[1]
}

c(u_list, omega_hat_list_euclid) %<-%
  get_omega_hat_list(euclidean_distance, psi_MLE, alpha, R, tol)
...
c(stat_fn_euclid, stat_fn_mod_u, stat_fn_u) %<-%
  map2(
...
ggplot() +
  ...
  geom_vline(
    aes(xintercept = as.numeric(MLE), color = Q),
    data = MLE_data,
    show.legend = FALSE
  ) +
  ...
  ylab("Log-Likelihood") +
  scale_x_continuous(expand = c(0, 0), limits = x_range) +
...
MLE_data |>
  select(Q, MLE) |>
  mutate(
    MLE = as.numeric(MLE) |> round(3),
    conf_int = map(conf_ints, \(x) paste0("(", x[1], ", ", x[2], ")")),
    length = map(conf_ints, diff)
  ) |>
  kbl(
    col.names = c("Q", "MLE", "95% Confidence Interval", "CI Length"),
    align = "c",
    caption = population
  ) |>
