...
integrated_log_likelihood_sims <- get_integrated_log_likelihood_sims(
  IL_preallocations,
  step_size,
  num_std_errors,
  num_chunks
...
integrated_log_likelihood_sims_file_path <- "seed={seed}_numsims={num_sims}_R={R}_tol={tol}_numse={num_std_errors}_stepsize={step_size}.Rda" |>
  glue::glue() |>
  paste0("Simulations/", population, "/Integrated Likelihood/IL_sims_", ... = _)
...
mod_integrated_log_likelihood_sims <- get_mod_integrated_log_likelihood_sims(
...
gen_PL_sims <- function(sims) {
  ...
  foreach(
    ...
    .multicombine = TRUE,
    .maxcombine = num_sims,
    .options.future = list(seed = TRUE, chunk.size = num_chunks)
  ) %dofuture%
    {
...
profile_log_likelihood_sims_file_path <- "seed={seed}_numsims={num_sims}_numse={num_std_errors}_stepsize={step_size}.Rda" |>
  glue::glue() |>
  paste0("Simulations/", population, "/Profile Likelihood/PL_sims_", ... = _)
...
filepaths <- list.files(
  "Simulations/Birds in Balrath Woods/Integrated Likelihood",
  pattern = ".*batch=.*"
) |>
  sort()
...
saveRDS(
  sims_list,
  "Simulations/Birds in Balrath Woods/Integrated Likelihood/IL_sims_seed=38497283_numsims=1000_R=250_numse=5_stepsize=0.01.Rda"
)

filepaths <- list.files(
  "Simulations/Birds in Balrath Woods/Modified Integrated Likelihood",
  pattern = ".*batch=.*"
) |>
  sort()
...
saveRDS(
  sims_list,
  "Simulations/Birds in Balrath Woods/Modified Integrated Likelihood/mod_IL_sims_seed=38497283_Q=nll_numsims=1000_R=250_numse=5_stepsize=0.01.Rda"
)
