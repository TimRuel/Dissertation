...
for (i in 9:10) {
  ...
  integrated_log_likelihood_sims_batch <- get_integrated_log_likelihood_sims(
    IL_preallocations[batch],
    step_size,
    num_std_errors,
    chunk_size
  )
  ...
  integrated_log_likelihood_sims_batch_file_path <- integrated_log_likelihood_sims_file_path |>
    paste0("_batch=", sprintf("%02d", i), ".Rda")

  saveRDS(
    integrated_log_likelihood_sims_batch,
    integrated_log_likelihood_sims_batch_file_path
  )
...
filepaths <- list.files(
  paste0("Simulations/", population, "/Integrated Likelihood"),
  pattern = ".*batch=.*"
) |>
  sort()
...
for (i in 1:10) {
  ...
  mod_integrated_log_likelihood_sims_batch <- get_mod_integrated_log_likelihood_sims(
    mod_IL_preallocations[batch],
    step_size,
    num_std_errors,
    chunk_size
  )
  ...
  saveRDS(
    mod_integrated_log_likelihood_sims_batch,
    mod_integrated_log_likelihood_sims_batch_file_path
  )
...
filepaths <- list.files(
  paste0("Simulations/", population, "/Modified Integrated Likelihood"),
  pattern = ".*batch=.*"
) |>
  sort()
