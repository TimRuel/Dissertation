...
get_pseudolikelihoods <- function(spline_models, MLE_data) {
  spline_models |>
    map2(MLE_data$Maximum, \(mod, maximum) \(psi) predict(mod, psi)$y - maximum)
}

get_confidence_intervals <- function(
  pseudolikelihoods,
  LL_df_long,
  MLE_data,
  alpha_levels,
    ...
    \(pseudolikelihood, name) {
      ...
      map_dfr(alpha_levels, \(alpha) {
        ...
        lower_bound <- tryCatch(
          uniroot(
            \(psi) pseudolikelihood(psi) + crit,
            interval = c(psi_endpoints[1], MLE)
          )$root,
        ...
        upper_bound <- tryCatch(
          uniroot(
            \(psi) pseudolikelihood(psi) + crit,
            interval = c(MLE, psi_endpoints[2])
          )$root,
...
make_kable_caption <- function(path) {
  ...
  # Extract experiment version
  experiment_version <- path_parts[grep("^exp_v", path_parts)]

  # Determine if it's a simulation or individual run
  ...
  } else if ("individual_runs" %in% path_parts) {
    run_id <- path_parts[grep("^run_", path_parts)]
...
get_LL_comparison_plot <- function(
  stat_fns,
  LL_df_long,
  MLE_data,
  alpha_levels,
