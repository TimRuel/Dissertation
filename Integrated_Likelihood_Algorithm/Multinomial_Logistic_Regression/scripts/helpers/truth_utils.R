...
Beta_0_constraint_fn <- function(params, X2_vals, entropy_range, Beta2) {
  ...
  return(c(
    entropy_range[1] - min(entropies),
    max(entropies) - entropy_range[2]
  ))
}
...
optimize_Beta_0 <- function(J, entropy_range, X2_support, reps, Beta2) {
  ...
  result <- nloptr::auglag(
    ...
    hin = function(params) {
      Beta_0_constraint_fn(params, X2_vals, entropy_range, Beta2)
    },
...
get_Beta_0 <- function(X1_levels, J, p, reps) {
  ...
  X1X2_interaction_names <- paste0(
    "X1",
    setdiff(X1_level_names, X1_ref_level)
  ) |>
    paste0(":X2")
...
get_experiment_parameters <- function(X1_levels, model_specs) {
  ...
  experiment_parameters <- list(
    true_params = list(Beta_0 = Beta_0, theta_0 = theta_0, H_0 = H_0),
    pY_0 = pY_0,
