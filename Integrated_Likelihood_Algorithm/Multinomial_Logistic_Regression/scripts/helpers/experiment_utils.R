...
log_likelihood <- function(Beta, X_design, Y_design) {
  ...
  Beta <- matrix(Beta, nrow = p, ncol = Jm1, byrow = FALSE)
...
likelihood <- function(Beta, X_design, Y_design) {
  exp(log_likelihood(Beta, X_design, Y_design))
}
...
get_psi_endpoints <- function(
  ...
  psi_hat_SE <- sqrt(
    sum(
      matrix(1 + log(theta_MLE)) %*% (1 + log(theta_MLE)) * sigma,
      na.rm = TRUE
    ) /
      n_h
  )
...
get_psi_grid <- function(psi_endpoints, step_size, J) {
  ...
  psi_grid <- seq(
    ceiling(left / step_size) * step_size,
    floor(right / step_size) * step_size,
    by = step_size
  )

  if (psi_endpoints[1] < lower) {
    psi_grid <- c(lower, psi_grid[psi_grid > lower])
  }
  if (psi_endpoints[2] > upper) {
    psi_grid <- c(psi_grid[psi_grid < upper], upper)
  }
...
get_fine_psi_grid <- function(psi_grid, psi_mode, fine_step_size, fine_window) {
  ...
  lower_bound <- safe_max(
    psi_grid[psi_grid <= (psi_mode - fine_window)],
    min(psi_grid)
  )
  upper_bound <- safe_min(
    psi_grid[psi_grid >= (psi_mode + fine_window)],
    max(psi_grid)
  )

  # Build fine psi values from lower_bound to upper_bound
  fine_grid <- seq(from = lower_bound, to = upper_bound, by = fine_step_size) |>
    round(8)
...
get_omega_hat_branch_mode <- function(
  omega_hat,
  X_design,
  Y_design,
  X_h_design,
  ...
  Beta_hat_obj_fn <- function(Beta) {
    Beta_hat_obj_fn_rcpp(Beta, X_design, omega_hat, Jm1, p, n)
  }

  omega_hat_branch <- function(psi) {
    Beta_hat_con_fn <- function(Beta) {
      Beta_hat_con_fn_rcpp(Beta, X_h_design, psi, Jm1, p)
    }

    Beta_hat <- get_Beta_hat_template(
      Beta_hat_obj_fn,
      Beta_hat_con_fn,
      c(omega_hat)
    )
...
get_branch_params <- function(
  ...
  omega_hat_eq_con_fn <- function(Beta) {
    omega_hat_eq_con_fn_rcpp(Beta, X_h_design, Jm1, p, psi_hat)
  }

  omega_hat_ineq_con_fn <- function(Beta) {
    omega_hat_ineq_con_fn_rcpp(Beta, X_design, Y_design, Jm1, p, n, threshold)
  }
  ...
  while (TRUE) {
    ...
    if (
      !is.null(omega_hat_candidate) &&
        abs(omega_hat_eq_con_fn(omega_hat_candidate)) <= 0.1 &&
        omega_hat_ineq_con_fn(omega_hat_candidate) <= 0
    ) {
      omega_hat_candidate <- matrix(
        omega_hat_candidate,
        nrow = p,
        ncol = Jm1,
        byrow = FALSE
      ...
      if (
        between(
          omega_hat_candidate_branch_mode,
          head(psi_grid, 1),
          tail(psi_grid, 1)
        )
      ) {
        Beta_hat_obj_fn <- function(Beta) {
          Beta_hat_obj_fn_rcpp(Beta, X_design, omega_hat_candidate, Jm1, p, n)
        }
...
get_log_L_bar <- function(IL_branches) {
  ...
  log_L_bar_df <- data.frame(psi = merged_df$psi, Integrated = log_L_bar)

  return(list(df = log_L_bar_df, branches_matrix = branches_matrix))
}
...
get_integrated_LL <- function(config, X_design, model_df) {
  ...
  n_h <- nrow(X_h_design)
  psi_endpoints <- get_psi_endpoints(
    psi_hat,
    Beta_MLE,
    X_h_design,
    num_std_errors,
  ...
  log_likelihood_fn <- function(Beta) {
    log_likelihood_rcpp(Beta, X_design, Y_design, Jm1, p, n)
  }
  ...
  result <- foreach(
  ...
  ) %dofuture%
    {
      ...
      Beta_hat_obj_fn <- function(Beta) {
        Beta_hat_obj_fn_rcpp(Beta, X_design, branch_params$omega_hat, Jm1, p, n)
      }
      get_Beta_hat <- function(con_fn, init_guess) {
        get_Beta_hat_template(Beta_hat_obj_fn, con_fn, init_guess)
      }
      con_fn_template <- function(Beta, psi) {
        Beta_hat_con_fn_rcpp(Beta, X_h_design, psi, Jm1, p)
      }
      ...
      list(IL_branch = IL_branch, branch_params = branch_params)
    }
...
compute_profile_branch <- function(
  direction,
  step_size,
  fine_step_size,
  fine_window,
...
get_profile_LL <- function(config, X_design, model_df) {
  ...
  Beta_hat_obj_fn <- function(Beta) {
    Beta_hat_obj_fn_rcpp(Beta, X_design, Beta_MLE, Jm1, p, n)
  }
  get_Beta_hat <- function(con_fn, init_guess) {
    get_Beta_hat_template(Beta_hat_obj_fn, con_fn, init_guess)
  }
...
get_report_objects <- function(run_dir) {
  ...
  return(list(MLE_data = MLE_data, conf_ints = conf_ints))
}
