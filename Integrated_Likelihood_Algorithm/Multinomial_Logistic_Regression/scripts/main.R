...
i_am(
  "Integrated_Likelihood_Algorithm/Polytomous_Logistic_Regression/scripts/main.R"
)
...
if (length(args) < 2) {
  stop(
    "Usage: Rscript main.R <experiment_id> [requested_cores] [sim_id] [run_id] [--force] [--non-interactive]"
  )
}

experiment_id <- args[1]
requested_cores <- if (length(args) >= 2 && !grepl("^--", args[2])) {
  as.integer(args[2])
} else {
  NULL
}
...
if (!file.exists(config_path)) {
  ...
  if (
    !should_proceed_or_abort(
      paste0(
        "Experiment config already exists for '",
        experiment_id,
        "'. Proceed with it?"
      ),
      force = force,
...
config_snapshot$optimization_specs <- c(
  config_snapshot$optimization_specs,
  opt_config
)
