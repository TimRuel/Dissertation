...
get_logistic_regression_model <- function(data) {
...
get_likelihood <- function(model = NULL, b = NULL, data = NULL) {
  exp(get_log_likelihood(model, b, data))
}

get_neg_log_likelihood <- function(model = NULL, b = NULL, data = NULL) {
  -get_log_likelihood(model, b, data)
}
...
get_omega_hat <- function(U, model, X_h) {
  U * get_Y_hat(model, X_h) / (t(matrix(c(1, X_h))) %*% U)[1]
}

get_omega_hat_list <- function(U_list, model, X_h) {
  purrr::map(U_list, \(U) get_omega_hat(U, model, X_h))
}
...
get_Beta_hat <- function(psi, omega_hat, model, X_h, N) {
  ...
  frac <- model.matrix(model) %*% omega_hat |> plogis()
  ...
  fit <- glm(
    response ~ . - successes - failures - 1,
    data = data,
    family = binomial(link = logit),
    offset = rep(intercept, nrow(Y))
  )
