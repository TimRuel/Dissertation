...
for (i in 1:length(psi1)) {
  ...
  for (j in 1:R) {
    ...
    theta_hat_p <- auglag(x0 = rdirichlet(1, rep(1, m)),
                          ...
                          heq = function(theta) c(sum(theta) - 1, g(theta) - psi1[i]),
                          lower = rep(0, m))$par
    
    # Evaluate likelihood function at optimal theta value and store result
    L1[j] <- likelihood(theta_hat_p)
  }
...
fit_P <- loess(Profile ~ psi, data = likelihood_dat)

crit <- qchisq(0.95, 1) / 2

x <- log(likelihood(theta.hat))
...
l <- uniroot(function(psi) predict(fit_P, psi) - x + crit,
             interval = c(psi1 %>% head(1), psi_max))$root
