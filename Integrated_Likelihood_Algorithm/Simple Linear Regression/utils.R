...
likelihood <- function(alpha, beta, sigma_squared, x, y) {
  exp(log_likelihood(alpha, beta, sigma_squared, x, y))
}

neg_log_likelihood <- function(alpha, beta, sigma_squared, x, y) {
  -log_likelihood(alpha, beta, sigma_squared, x, y)
}
  ...
  MC_params$method |>
    ...
    switch(
      ...
      basic_IS = {
            ...
            log_p <- MC_params$nominal$density |>
              do.call(
                args = c(
                  x = list(u),
                  log = TRUE,
                  MC_params$nominal$density_params
                )
              ) |>
              sum()
            ...
            log_q <- MC_params$importance$density |>
              do.call(
                args = c(
                  x = list(u),
                  log = TRUE,
                  MC_params$importance$density_params
                )
              ) |>
              sum()
        ...
        if (MC_params$method == "self_norm_IS") {
          log_w <- log_w - matrixStats::logSumExp(log_w) + log(length(u_list))
        }
...
get_omega_hat_list <- function(u_list, psi_hat, x_h) {
  purrr::map(u_list, \(u) Q(u, psi_hat, x_h))
}
...
get_theta_hat <- function(init_guess, psi, omega_hat, x, x_h) {
  ...
  f <- function(theta) {
    n *
      ...
      sum(
        omega_hat[3] +
          ((omega_hat[1] - theta[1]) + (omega_hat[2] - theta[2]) * x)^2
      ) /
        theta[3]
  }
...
accumulate_theta_hats <- function(
  psi_grid_list,
  omega_hat,
  x,
  x_h,
...
get_log_L_tilde <- function(
  ...
  theta_hat_method |>
    ...
    switch(
      accumulate = accumulate_theta_hats(
        psi_grid_list,
        omega_hat,
        x,
        x_h,
    ...
    purrr::map_dbl(\(theta_hat) {
      log_likelihood(theta_hat[1], theta_hat[2], theta_hat[3], x, y)
    })
}

get_log_L_tilde_mat <- function(
  psi_grid_list,
  omega_hat_list,
  chunk_size,
  theta_hat_method,
...
get_log_L_bar <- function(log_L_tilde_mat, log_w, MC_params) {
  ...
  return(list(
    log_weighted_vals = log_weighted_vals,
    estimate = estimate,
...
get_log_integrated_likelihood <- function(
  ...
  log_L_tilde_mat <- get_log_L_tilde_mat(
    psi_grid_list,
    omega_hat_list,
    chunk_size,
    theta_hat_method,
