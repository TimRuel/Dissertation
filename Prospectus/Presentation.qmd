...
- **Parametric**
  - The dimension of the parameter space is finite. That is, $\Theta \subseteq \mathbbm{R}^d$ where $d \in \mathbb{Z}^+$.
- **Identifiable**: 
...
- The distributions in $\mathcal{P}$ have a common support $\mathcal{X} = \{x: p_{\theta}(x) > 0 \}\subseteq \mathbbm{R}$ not depending on $\theta$.
...
- $\Theta$ is a compact and convex subset of $\mathbbm{R}^d$.
...
- Sufficient condition: $$\frac{d}{d \theta}\int_{\mathbbm{R}} p(x; \theta) dx = \int_{\mathbbm{R}} \frac{\partial}{\partial \theta}p(x; \theta) dx.$$
...
- Sufficient condition: $$\frac{d^2}{d \theta^2}\int_{\mathbbm{R}} p(x; \theta) dx = \int_{\mathbbm{R}} \frac{\partial^2}{\partial \theta^2}p(x; \theta) dx.$$
...
- Suppose $\psi$ is an implicit parameter of interest so that $\psi = \varphi(\theta)$ for some function $\varphi: \Theta \to \mathbbm{R}$. Define $$\Omega_{\psi} = \Big\{\omega \in \Theta: \varphi(\omega) = \psi\Big\}.$$
...
- The full parameter is $\theta = (\theta_1, ..., \theta_m)$ and the parameter space $\Theta$ is the probability simplex in $\mathbbm{R}^m$.
...
- Set $\check{L}(u) := L(u)$ and choose $\check{\pi}(u)$ to be the symmetric Dirichlet distribution on the probability simplex in $\mathbbm{R}^d$ with $\alpha_j = 1$ for all $j$.
