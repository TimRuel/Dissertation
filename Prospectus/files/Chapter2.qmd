...
\begin{enumerate}[label = RC\arabic*)]
  ...
  \item There exists a random function $M(x)$ not depending on $\theta$ such that $\mathbb{E}[M(X); \theta_0] < \infty$ and $\underset{\theta \in \Theta^*}{\sup} |\nabla^2\mathcal{S}(\theta;x)_{ijk}| \leq M(x)$ for all integers $1 \leq i, j, k \leq d$. 
\end{enumerate}
