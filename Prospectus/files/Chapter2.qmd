...
\begin{enumerate}[label = RC\arabic*)]
  ...
  \item $p(x; \theta)$ is twice continuous with respect to $\theta$ for all $\mathbf{x}$.
...
RC1) is included merely to simplify calculations and frame our discussion in the context of a standard case in which likelihood theory holds. RC2-4) exist to ensure that there is no issue with defining derivatives of the log-likelihood function. RC5) implies that first and second partial derivatives with respect to $\theta$ may be passed under the integral sign in $\int p(x; \theta) dx$, and that the remainder in term in the Taylor series expansion of the score function centered at $\theta_0$ is negligible. Finally, RC6) is necessary to guarantee that it is always possible to solve the likelihood equation for $\hat{\theta}$.
