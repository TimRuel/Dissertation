...
Single-index asymptotic theory describes the behavior of likelihood functions as the sample size ($n$) grows to infinity while the dimension of the nuisance parameter ($m$) remains fixed. The aim of this section is to present a basic overview of the theory's results so that the reader will have a readily available baseline against which to compare the results of the following section discussing two-index asymptotic theory.
...
Let $X_1, ..., X_n$ be IID random variables with density function $p(\mathbf{x}; \theta)$ and let $\hat{\theta}$ denote the maximum likelihood estimate for $\theta_0$. The likelihood function for $\theta$ based on the data $\mathbf{x} = (x_1, ..., x_n)$ is $$L(\theta; \mathbf{x}) \equiv p(\mathbf{x}; \theta) = \prod_{i=1}^n p(x_i; \theta).$$ We may write the log-likelihood function as 
$$
\begin{aligned}
...
eq-score_additive implies that a Taylor series expansion of $\mathcal{S}(\theta; \mathbf{x})$ will be equal to the sum of the Taylor series expansions of its individual contributions, plus a remainder term that depends on $n$. Since each we have assumed each $x_i$ is identically distributed, it suffices to consider the expansion for an arbitrary contribution, $\mathcal{S}(\theta; x_i)$.

It will be helpful to recall that because $\theta$ is a vector, the $k$-th derivative of the log-likelihood with respect to $\theta$ will be a $k$-dimensional array having $d$ entries along each of its $k$ indices. Consequently, the score function (i.e. the first derivative of the log-likelihood) will itself be a $d$-dimensional vector, and its first and second derivatives will be a $d \times d$ matrix and a three-dimensional $d \times d \times d$ array, respectively.

Because  $\mathcal{S}(\theta; x_i)$ is vector-valued function, for $\theta, \theta_0 \in \Theta^*$ there does not in general exist a $\theta^*$ on the line segment between $\theta$ and $\theta_0$ such that $$\mathcal{S}(\theta; x_i) = \mathcal{S}(\theta_0; x_i) + \nabla\mathcal{S}(\theta^*; x_i)(\theta - \theta_0)$$ as is normally required in order to bound the error term of a Taylor series expansion (@Feng2014). However, for an individual component $\mathcal{S}_j(\theta; x_i)$, there does exist a $\theta_j^*$ on the line segment between $\theta_j$ and $\theta_{0j}$ such that $$\mathcal{S}_j(\theta_j; x_i) = \mathcal{S}_j(\theta_{0j}; x_i) + \frac{\partial}{\partial \theta}\mathcal{S}_j(\theta^*; x_i)(\theta_j - \theta_{0j}).$$

Consequently, we will first focus our attention on a single component of the score function contribution $\mathcal{S}(\theta; x_i)$.

We will use tensor product notation to express a multiplication involving this three-dimensional array in the expansion. See Appendix \ref{appendix:A} for a review of tensor product notation as well as Taylor series in general.

By the fifth regularity condition in Section 1.2.2, we may assume the components of $\nabla^3\ell_i(\theta)$ are bounded by a random function $M(x_i)$ for all $\theta$ in a neighborhood $N_{\theta_0}$ of $\theta_0$. Therefore, for all $\theta \in N_{\theta_0}$, a first-order Taylor expansion of $\mathcal{S}_i(\theta)$ around the point $\theta = \theta_0$ is given as follows: $$\mathcal{S}_i(\theta) = \mathcal{S}_i(\theta_0) + \nabla \mathcal{S}_i(\theta_0)(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{2} \nabla^2 \mathcal{S}_i(\theta^*)\cdot(\theta - \theta_0)^{\otimes 2}$, for some $\theta^* \in \Theta^*$.
