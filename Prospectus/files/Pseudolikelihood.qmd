...
$$\begin{aligned}
S_j(\theta; x_i) &= S_j(\theta_0; x_i) + \sum_{|\symbf{\alpha}| = 1}\frac{D^{\symbf{\alpha}} S_j(\theta; x_i) \Big|_{\theta = \theta_0}}{\symbf{\alpha}!}(\theta - \theta_0)^{\symbf{\alpha}} + \sum_{|\symbf{\alpha}| = 2} \frac{D^{\symbf{\alpha}} S_j(\theta; x_i) \Big|_{\theta = \bar{\theta}}}{\symbf{\alpha}!}(\theta - \theta_0)^{\symbf{\alpha}} \\
                 &= S_j(\theta_0; x_i) + \sum_{|\symbf{\alpha}| = 1}D^{\symbf{\alpha}} S_j(\theta; x_i)\Big|_{\theta = \theta_0}(\theta - \theta_0)^{\symbf{\alpha}} + \frac{1}{2}\sum_{|\symbf{\alpha}| = 2} D^{\symbf{\alpha}} S_j(\theta; x_i)\Big|_{\theta = \bar{\theta}}(\theta - \theta_0)^{\symbf{\alpha}} \\
                 &= S_j(\theta_0; x_i) + \sum_{k = 1}^d\frac{\partial S_j(\theta; x_i)}{\partial \theta_k}\Bigg|_{\theta = \theta_0}(\theta_k - \theta_{0k}) + \frac{1}{2}\sum_{k = 1}^d\sum_{l = 1}^d\frac{\partial^2 S_j(\theta; x_i)}{\partial \theta_k \theta_l}\Bigg|_{\theta = \bar{\theta}}(\theta_k - \theta_{0k})(\theta_l - \theta_{0l}).
\end{aligned}
$$ 

Note that the neighborhood of a point is always a convex set, and so by definition it must contain the whole line segment connecting any two points within it. In particular, we must have $LS(\theta, \theta_0) \subset \Theta^*$, and therefore $\bar{\theta} \in \Theta^*$. Therefore, since the score function is the first derivative of the log-likelihood with respect to $\theta$, it follows from condition C5) that there exists a random function $M(\mathbf{x})$ with finite expectation such that $$\underset{\underset{|\symbf{\alpha}| = 2}{\theta \in N_r(\theta_0)}}{\sup} \Big|D^{\symbf{\alpha}} S_j(\bar{\theta}; x_i)\Big| \leq M(\mathbf{x}).$$
