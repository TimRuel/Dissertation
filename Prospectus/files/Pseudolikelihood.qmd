...
By regularity conditions C1) and C2) that we assumed in Section 1.2.2, there exists an open set $\Theta^* \subseteq \Theta$ containing $\theta_0$ and for which $p(\mathbf{x}; \theta)$ is three times continuously differentiable with respect to $\theta$ for all $\theta \in \Theta^*$. It immediately follows that $S_j(\theta; x_i)$ is twice continuously differentiable with respect to $\theta$ on $\Theta^*$. Since $\theta_0$ is an interior point of $\Theta^*$, there must exist a neighborhood $N_r(\theta_0) \subseteq \Theta^*$ for which $S_j(\theta; x_i)$ is twice continuously differentiable with respect to $\theta$ for all $\theta \in N_r(\theta_0)$ as well. By Taylor's theorem, it follows that for any $\theta \in N_r(\theta_0)$ there exists a point $\bar{\theta}$ on the line segment $LS(\theta, \theta_0)$ such that $$S_j(\theta; x_i) = S_j(\theta_0; x_i) + \sum_{|\symbf{\alpha}| = 1}\frac{D^{\symbf{\alpha}} S_j(\theta_0; x_i)}{\symbf{\alpha}!}(\theta - \theta_0)^{\symbf{\alpha}} + \sum_{|\symbf{\beta}| = 2} R_{\symbf{\beta}}(\theta),$$ where each remainder term $R_{\symbf{\beta}}(\theta)$ satisfies $$R_{\symbf{\beta}}(\theta) = \frac{D^{\symbf{\beta}} S_j(\bar{\theta}; x_i)}{\symbf{\beta}!}(\theta - \theta_0)^{\symbf{\beta}}.$$ Alternatively, we can write this as $$S_j(\theta; x_i) = S_j(\theta_0; x_i) + \sum_{k = 1}^d\frac{\partial S_j(\theta_0; x_i)}{\partial \theta_k}(\theta_k - \theta_{0k}) + \sum_{k = 1}^d\sum_{l = 1}^d\frac{\partial^2 S_j(\theta_0; x_i)}{\partial \theta_k \theta_k}(\theta_k - \theta_{0k})(\theta_l - \theta_{0l}),$$



By condition C5), there exists a neighborhood $N_r(\theta_0) \subseteq \Theta^*$ of $\theta_0$ such that for all $\theta \in N_r(\theta_0)$, $\underset{\theta \in N_r(\theta^*)}{\sup} \Big|D^{\symbf{\alpha}} S_j(\theta; x_i)\Big| \leq M(\mathbf{x})$ for all $\symbf{\alpha}$ such that $|\symbf{\alpha}| = 2$. Then for any $\theta \in N_r(\theta_0)$, there exists 

Let $N_r(\theta_0) \subseteq \Theta^*$ be some neighborhood of $\theta_0$.

For all $\theta \in N_r(\theta_0)$, there exists $\bar{\theta} \in LS(\theta, \theta_0)$ such that a first-order Taylor series expansion of $\mathcal{S}_j(\theta)$ around the point $\theta = \theta_0$ may be given as follows: $$\mathcal{S}_j(\theta) = \mathcal{S}_j(\theta_0) + \nabla \mathcal{S}_j(\theta_0)(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{2} \nabla^2 \mathcal{S}_i(\theta^*)\cdot(\theta - \theta_0)^{\otimes 2}$, for some $\theta^* \in \Theta^*$.
 
 By the regul in Section 1.2.2, we may assume the components of $\nabla^3\ell_i(\theta)$ are bounded by a random function $M(x_i)$ for all $\theta$ in a neighborhood $N_{\theta_0}$ of $\theta_0$.
