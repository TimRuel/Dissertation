...
Assume that the regularity conditions established in Section 1.2.2 apply to our model. Conditions C2) and C3) stipulate that there exists an open subset $\Theta^* \subseteq \Theta$ containing $\theta_0$ for which $p(\mathbf{x}; \theta)$ is three times continuously differentiable with respect to $\theta$ for all $\theta \in \Theta^*$. It immediately follows that $S_j(\theta; x_i)$ is twice continuously differentiable with respect to $\theta$ on $\Theta^*$. Since $\theta_0$ is an interior point of $\Theta^*$, there must exist a neighborhood $N_r(\theta_0) \subseteq \Theta^*$ for which $S_j(\theta; x_i)$ is twice continuously differentiable with respect to $\theta$ for all $\theta \in N_r(\theta_0)$ as well. Then for all $\theta \in N_r(\theta_0)$, the first order Taylor series polynomial of $S_j(\theta; x_i)$ around $\theta_0$ is given by $$T_j(\theta; x_i) = S_j(\theta_0; x_i) + \nabla S_j(\theta_0; x_i)^T(\theta - \theta_0).$$

Let $R(\theta)$ denote the difference between $S_j(\theta; x_i)$ and $T_j(\theta; x_i)$ so that $$S_j(\theta; x_i) = T_j(\theta; x_i) + R(\theta).$$ By condition C5), there exists a random function $M(\mathbf{x})$ with finite expectation that depends (or at least is allowed to depend) on the data but not on $\theta$ for which $$\underset{\theta \in N_r(\theta_0)}{\sup}\Big|D^{\symbf\alpha}S_j(\theta; x_i)\Big| \leq M(\mathbf{x})$$ for all $\symbf\alpha$ such that $|\symbf \alpha| = 2$.
