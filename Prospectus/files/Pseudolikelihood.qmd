...
The likelihood function for $\theta$ based on $\mathbf{x} = (x_1, ..., x_n)$ is $$L(\theta; \mathbf{x}) \equiv p(\mathbf{x}; \theta) = \prod_{i=1}^n p(x_i; \theta).$$ We may therefore write the log-likelihood function as 
$$
\begin{aligned}
...
@eq-score_additive implies that a Taylor series expansion of $\mathcal{S}(\theta; \mathbf{x})$ will be equal to the sum of the Taylor series expansions of its individual contributions, plus a remainder term that depends on $n$. We have assumed each $x_i$ is identically distributed, so it suffices to consider the expansion for an arbitrary contribution, $\mathcal{S}(\theta; x_i)$.

Before proceeding, it will be helpful to review the theorem 

Since $\theta$ is a vector, the $k$-th derivative of the log-likelihood with respect to $\theta$ will be a $k$-dimensional array having $d$ entries along each of its $k$ indices. In particular, the score function (i.e. the first derivative of the log-likelihood) will be a $d \times 1$ vector, $$\mathcal{S}(\theta; x_i) = \begin{pmatrix} \mathcal{S}_1(\theta; x_i) \\ \vdots \\ \mathcal{S}_d(\theta; x_i) \end{pmatrix},$$ and its first and second derivatives will be a $d \times d$ matrix and a three-dimensional $d \times d \times d$ array, respectively.

For a scalar-valued function $f: \mathbb{R}^p \to \mathbb{R}$ that is $k+1$-times continuously differentiable in a neighborhood $N_r(\mathbf{x}_0)$ of a point $\mathbf{x}_0 \in \mathbb{R}^d$, Taylor's theorem states that for any $\mathbf{x} \in N_r(\mathbf{x}_0)$, there exists another point $\mathbf{x}^*$ on the line segment connecting $\mathbf{x}$ and $\mathbf{x}_0$ such that $$\mathbb{}$$

For $\theta, \theta_0 \in \Theta^*$ there does not in general exist a $\theta^*$ on the line segment between $\theta$ and $\theta_0$ such that $$\mathcal{S}(\theta; x_i) = \mathcal{S}(\theta_0; x_i) + \nabla\mathcal{S}(\theta^*; x_i)(\theta - \theta_0)$$ as is normally required in order to bound the error term of a Taylor series expansion (@Feng2014). However, for an individual component $\mathcal{S}_j(\theta; x_i)$, there does exist a $\theta_j^*$ on the line segment between $\theta_j$ and $\theta_{0j}$ such that $$\mathcal{S}_j(\theta_j; x_i) = \mathcal{S}_j(\theta_{0j}; x_i) + \frac{\partial}{\partial \theta}\mathcal{S}_j(\theta^*; x_i)(\theta_j - \theta_{0j}).$$ Consequently, we will first focus our attention on a single component of the score function contribution $\mathcal{S}(\theta; x_i)$.
