...
$$
\begin{aligned}
\mathbb{E}_{\theta}[\ell_{\theta \theta}(\theta; x)] + \mathbb{E}_{\theta}\Big[\big(\ell_{\theta}(\theta; x) \big)^2\Big] &= \mathbb{E}_{\theta}\Bigg[\frac{\frac{\partial^2}{\partial \theta^2} f(x; \theta)}{f(x; \theta)}\Bigg] \\
...
Suppse $X_1, ..., X_n$ are independent draws from some distribution $P_{\theta}$ with parameter $\theta$ and density function $f(x; \theta)$. Let $\hat{\theta}_n$ denote the maximum likelihood estimate for $\theta$.^[The existence of $\hat{\theta}$ is guaranteed assuming the regularity of the model (see Section 2.1.4.).] A second-order Taylor expansion of the score function $\ell_{\theta}(\theta)$ around the point $\theta = \hat{\theta}_n$ is given as follows:
