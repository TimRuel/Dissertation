...
\end{aligned}
$$
Similarly, we may then write the score function as
$$
\begin{aligned}
u(\theta; x) &= \nabla \ell(\theta; x) \\
             &= 
...
A second-order Taylor expansion of the score function $u(\theta)$ around the point $\theta = \hat{\theta}_n$ is given as follows:

$$u(\theta) = u(\hat{\theta}_n) + u_{\theta}(\hat{\theta}_n)(\theta - \hat{\theta}_n) + \frac{1}{2}u_{\theta\theta}(\hat{\theta}_n)(\theta - \hat{\theta}_n)^2 + R_n(\theta),$$ where $R_n(\theta) = \frac{1}{6}u_{\theta\theta\theta}(\theta^*)(\theta - \hat{\theta}_n)^3$, for some $\theta^* \in \Theta$.
