...
where $u_i(\theta) \equiv u(\theta; x_i)$. In other words, the score function for the parameter $\theta$ based on data $x_1, ..., x_n$ can be written as the sum of individual contributions $u_i(\theta)$, $i = 1,..., n$, where each $u_i(\theta)$ is the score function for $\theta$ based only on data point $x_i$. 

Let's focus our attention on a single contribution, $u_i(\theta)$.A second-order Taylor expansion of $u_i(\theta)$ around the point $\theta = \theta_0$ is given as follows:

$$u_i(\theta) = u_i(\theta_0) + \nabla u_i(\theta_0)^T(\theta - \theta_0) + \frac{1}{2} (\theta - \theta_0)^T\nabla^2 u_i(\theta_0)(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{6}\nabla^3u_i(\theta^*)[\theta - \theta_0, \theta - \theta_0, \theta - \theta_0]$, for some $\theta^* \in \Theta_0$.
