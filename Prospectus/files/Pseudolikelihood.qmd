...
For all $\theta \in N_{\theta_0}$, there exists $\bar{\theta} \in LS(\theta, \theta_0)$ such that a first-order Taylor series expansion of $\mathcal{S}_j(\theta)$ around the point $\theta = \theta_0$ may be given as follows: $$\mathcal{S}_j(\theta) = \mathcal{S}_j(\theta_0) + \nabla \mathcal{S}_j(\theta_0)(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{2} \nabla^2 \mathcal{S}_i(\theta^*)\cdot(\theta - \theta_0)^{\otimes 2}$, for some $\theta^* \in \Theta^*$.
