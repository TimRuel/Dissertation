...
Let's focus our attention on a single contribution, $\mathcal{S}_i(\theta)$. A second-order Taylor expansion of $\mathcal{S}_i(\theta)$ around the point $\theta = \theta_0$ is given as follows: $$\mathcal{S}_i(\theta) = \mathcal{S}_i(\theta_0) + \nabla \mathcal{S}_i(\theta_0)^T(\theta - \theta_0) + \frac{1}{2} (\theta - \theta_0)^T\nabla^2 \mathcal{S}_i(\theta_0)(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{6}\nabla^3\mathcal{S}_i(\theta^*)[\theta - \theta_0, \theta - \theta_0, \theta - \theta_0]$, for some $\theta^* \in \Theta^*$.
