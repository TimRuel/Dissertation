...
\mathcal{S}(\theta; \mathbf{x}) &= \frac{\partial}{\partial \theta} \ell(\theta; \mathbf{x}) \\
             &= \frac{\partial}{\partial \theta} \sum_{i=1}^n \ell_i(\theta) \\
             &= \sum_{i=1}^n \frac{\partial}{\partial \theta} \ell_i(\theta) \\
             &= \sum_{i=1}^n \mathcal{S}_i(\theta),
\end{aligned}
$$
where $\mathcal{S}_i(\theta) \equiv \mathcal{S}(\theta; x_i)$. In other words, the score function for the parameter $\theta$ based on data $x_1, ..., x_n$ can be written as the sum of individual contributions $\mathcal{S}_i(\theta)$, $i = 1,..., n$, where each $\mathcal{S}_i(\theta)$ is the score function for $\theta$ based only on observation $x_i$. Note that these individual contributions are all independent from one another, a consequence of our earlier assumption that the observations themselves are independent.

We will now consider a first-order Taylor series expansion of an arbitrary contribution, $\mathcal{S_i}(\theta)$. Because $\theta$ is a multi-dimensional parameter, the first and second derivatives of $\mathcal{S_i}(\theta)$ with respect to it will be a matrix and a three-dimensional array, respectively (remember that the score function is itself a vector when $\theta$ is multi-dimensional). We will use tensor product notation to express a multiplication involving this three-dimensional array in the expansion. See \ref{appendix:A} for a review of tensor product notation as well as Taylor series in general.

By the fifth regularity condition in Section 1.2.2, we may assume the components of $\nabla^3\ell_i(\theta)$ are bounded by a random function $M(x_i)$ for all $\theta$ in a neighborhood $N_{\theta_0}$ of $\theta_0$. Therefore, for all $\theta \in N_{\theta_0}$, a first-order Taylor expansion of $\mathcal{S}_i(\theta)$ around the point $\theta = \theta_0$ is given as follows: $$\mathcal{S}_i(\theta) = \mathcal{S}_i(\theta_0) + \nabla \mathcal{S}_i(\theta_0)\cdot(\theta - \theta_0) + R(\theta),$$ where $R(\theta) = \frac{1}{2} \nabla^2 \mathcal{S}_i(\theta^*)\cdot(\theta - \theta_0)^{\otimes 2}$, for some $\theta^* \in \Theta^*$.
