...
Let $R(\theta)$ denote the difference between $S_j(\theta; x_i)$ and $T_j(\theta; x_i)$ so that $$S_j(\theta; x_i) = T_j(\theta; x_i) + R(\theta).$$ By condition C5), there exists a random function $M(\mathbf{x})$ that depends (or at least is allowed to depend) on the data but not on $\theta$ such that $$\Big|D^{\symbf\alpha}S_j(\theta; x_i)\Big| \leq M(\mathbf{x})$$
