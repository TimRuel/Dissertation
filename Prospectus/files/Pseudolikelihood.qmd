...
Since $\theta_0$ is an interior point of $\Theta^*$, there must exist a neighborhood $N_r(\theta_0) \subseteq \Theta^*$ for which $S_j(\theta; x_i)$ is twice continuously differentiable with respect to $\theta$ for all $\theta \in N_r(\theta_0)$ as well. Taylor's theorem then states that for any $\theta \in N_r(\theta_0)$ there exists a point $\bar{\theta}$ on the line segment $LS(\theta, \theta_0)$ such that 
$$\begin{aligned}
