...
$$\mathcal{I}(\theta) = \mathcal{I}(\theta; \mathbf{x}) = -\nabla^2\ell(\theta; \mathbf{x}) = -
\begin{pmatrix} 
...
$$
\begin{aligned}
\mathscr{I}(\theta) &= \mathbb{V}\big[\mathcal{S}(\theta; X)| \> \theta\big] &&(\text{by Eq. 2.2.4})\\
                    &= \text{Cov}\bigg[\begin{pmatrix} \frac{\partial \ell}{\partial\theta_1} \cdots  \frac{\partial \ell}{\partial\theta_d}  \end{pmatrix}^T| \> \theta\bigg] &&(\text{by Eq. 2.2.2})\\
                    &= \begin{pmatrix} 
...
\end{aligned}
$${#eq-exp_information2}
We have omitted the "conditional on $\theta$" arguments inside each of the variance and covariance operators in the final line above for the sake of avoiding overly cluttered notation, but it should be understood that all of these quantities are really integrals of the form $\int p(x; \theta) dx$.

Note that $X$ can represent an scalar observation drawn from a single distribution or it can represent a vector of observations drawn from a group of distributions. In the latter case, if the observations are independent, the Fisher information of the whole sample is equal to the sum of the Fisher information values for each of the observations individually. That is, for independent random variables $X_1, ..., X_n$, $$\mathscr{I}_X(\theta) = \sum_{i=1}^n \mathscr{I}_{X_i}(\theta).$${#eq-exp_information3} If the random variables are also identically distributed, then the Fisher information of the entire sample is simply equal to the Fisher information for a single observation drawn from the common distribution multiplied by a factor of $n$: $$\mathscr{I}_X(\theta) = n\mathscr{I}_{X_1}(\theta).$${#eq-exp_information4}

\section{Regularity Conditions}

As a consequence of the random variable interpretation of likelihood-based quantities such as the score function and the MLE for $\theta_0$, a natural line of inquiry to investigate is how the behavior of these random variables changes as the sample size $n$ increases. Of particular interest is the distribution to which the maximum likelihood estimate converges as $n$ tends toward infinity, if indeed one exists. To that end, it will be useful to establish some *regularity conditions* for our models. We can think of these conditions as being assumptions similar to those we discussed in the introduction to this paper that, when satisfied, endow our models with certain properties that enable us to determine the aforementioned distribution. 
...
Consider the case in which $\theta$ is a scalar parameter for a regular model. Using the symbol $\nabla$ to denote gradients is not necessary here since $\theta$ is no longer a vector; instead, we will use subscripts to denote partial derivatives with respect to $\theta$. For example, we will write the first derivative of the log-likelihood with respect to $\theta$ as $\ell_{\theta}(\theta;x ) = \frac{\partial}{\partial \theta} \ell(\theta; x)$. Taking the expectation of $\ell_{\theta}(\theta; x)$ gives

$$
\begin{aligned}
\mathbb{E}\big[\ell_{\theta}(\theta; X) | \> \theta \big]  &= \mathbb{E}\bigg[\frac{\partial}{\partial \theta} \ell(\theta; X)| \> \theta\bigg] \\
                                                           &= \int_{\mathbb{R}} \bigg[\frac{\partial}{\partial \theta} \log p(x; \theta)\bigg] p(x; \theta) dx \\
...
\end{aligned}
$$
Therefore, $$\mathbb{E}\big[\ell_{\theta}(\theta; X)| \> \theta \big] = 0.$${#eq-BI1} @eq-BI1 is called the first Bartlett identity. In words, it states that the expectation of the first derivative of the log-likelihood function of a statistical model with respect to the full model parameter will always be 0. Since $\ell_{\theta}$ is also defined as the score, any function that satisfies the first Bartlett identity is said to be *score-unbiased*.

Recall from Section 3.2 our definition of the expected information of the parameter $\theta$ as the variance of the score. For any modelNote that the second term on the left-hand side can be further rewritten as 
$$
\begin{aligned}
\mathbb{E}\Big[\big(\ell_{\theta}(\theta; x) \big)^2\Big| \> \theta\Big] &= \mathbb{V}[\ell_{\theta}(\theta; x)| \> \theta] +\Big(\mathbb{E}\big[\ell_{\theta}(\theta; x)| \> \theta \big]\Big)^2 \\
                                                                     &= \mathbb{V}[\ell_{\theta}(\theta; x)| \> \theta]. &&(\text{by the first Bartlett identity}) \\
                                                                     &= \mathbb{V}[\mathcal{S}(\theta; x)| \> \theta] \\
                                                                     &= \mathscr{I}(\theta).
\end{aligned}
$$
...
\ell_{\theta \theta}(\theta; x) &= \frac{\partial^2}{\partial \theta^2} \ell(\theta; x)\\
                                ...
                                &= \frac{\frac{\partial^2}{\partial \theta^2} p(x; \theta)}{p(x; \theta)} - \big[\ell_{\theta}(\theta; x) \big]^2. 
\end{aligned} 
$$
Rearranging terms and taking expectations yields 
$$
\begin{aligned}
\mathbb{E}[\ell_{\theta \theta}(\theta; x)| \> \theta] + \mathbb{E}\Big[\big(\ell_{\theta}(\theta; x) \big)^2\Big| \> \theta\Big] &= \mathbb{E}\Bigg[\frac{\frac{\partial^2}{\partial \theta^2} p(x; \theta)}{p(x; \theta)}\Bigg| \> \theta\Bigg] \\
            &= \int_{\mathbb{R}} \Bigg[\frac{\frac{\partial^2}{\partial \theta^2} p(x; \theta)}{p(x; \theta)}\Bigg] p(x; \theta) dx \\
...
\end{aligned}
$$
Therefore, $$\mathbb{E}[\ell_{\theta \theta}(\theta; x)| \> \theta] + \mathbb{E}\Big[\big(\ell_{\theta}(\theta; x) \big)^2| \> \theta\Big] = 0.$${#eq-BI2}

@eq-BI2 is called the second Bartlett identity. 
Another name for this quantity is the *expected information*. 

It follows from the second Bartlett identity that $$\mathbb{E}[-\ell_{\theta \theta}(\theta; x)| \> \theta] = \mathbb{V}[\ell_{\theta}(\theta; x); \theta].$${#eq-BI3} 
...
To do this, first note that the likelihood function for $\theta$ based on $\mathbf{x} = (x_1, ..., x_n)$ as $$L(\theta; \mathbf{x}) = p(\mathbf{x}; \theta) = \prod_{i=1}^n p(x_i; \theta).$$ We may therefore write the log-likelihood function as 
$$
\begin{aligned}
