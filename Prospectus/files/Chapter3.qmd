...
Finally, **RC8-9)** play an important role in ensuring the existence and uniqueness of the maximum likelihood estimator of a regular model's parameter.
...
Assume the regularity conditions of the previous section apply to our model and let $\hat{\symbf{\theta}}$ denote the MLE for $\symbf{\theta}_0$. 

The traditional method for analyzing the asymptotic behavior of $\hat{\symbf{\theta}}$ is to use a first-order Taylor series expansion of the score function.^[See Appendix \ref{appendix:A} for a review of Taylor's theorem and the conditions under which it is satisfied.] 
