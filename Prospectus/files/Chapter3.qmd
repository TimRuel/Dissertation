...
In the general case in which $\theta$ is a $d$-dimensional vector, where $d$ is an integer greater than 1, it follows that the first partial derivative of the log-likelihood with respect to $\theta$ will also be a $d$-dimensional vector, the second partial derivative will be a $d \times d$ matrix, the third derivative will be a $d \times d \times d$ array, and so forth. Consequently, we will use gradient notation (e.g. $\nabla$, $\nabla^2$, etc.) to emphasize the vector nature of these results along with a subscript that indicates the variable with respect to which the partial derivatives are being taken. 
...
$$\mathcal{S}(\theta) = \mathcal{S}(\theta; \mathbf{x}_n) = \nabla_{\theta}\ell(\theta; \mathbf{x}_n) = \begin{pmatrix} \frac{\partial \ell}{\partial\theta_1} \\ \vdots \\ \frac{\partial \ell}{\partial\theta_d}  \end{pmatrix}, \> \> \theta \in \Theta.$${#eq-score} 
...
\begin{enumerate}[label = \textbf{RC\arabic*)}]
  ...
  \item The distributions in $\mathcal{P}$ have a common support $\mathcal{X} = \{x: p_{\theta}(x) > 0 \}\subseteq \mathbb{R}$ not depending on $\theta$.
...
**RC2)** is the identifiability assumption we mentioned in Chapter 2. It is particularly important for our discussion of maximum likelihood estimation that $p(\mathbf{x}_n; \theta) = p(\mathbf{x}_n; \theta_0)$ implies $\theta = \theta_0$. **RC3)** requires that the distributions in $\mathcal{P}$ be supported on a common subset of the real line, and the definition of this subset cannot depend on $\theta$. This is to prevent situations in which, for example, the event $\{X_i \leq x_i \}$ occurs with positive probability when $\theta = \theta_1$ but not $\theta = \theta_2$. 

**RC4)** guarantees the existence of a neighborhood around $\theta_0$ on which partial derivatives with respect to $\theta$ can be defined. **RC5)** goes on to assert directly the existence and continuity of the first two of those partial derivatives, which is necessary for defining second-order Taylor series expansions of the score function around $\theta_0$. **RC6)** is necessary to ensure the remainder terms in these expansions are negligible.

**RC7)** grants us the ability to freely interchange integration and second-order partial differentiation with respect to $\theta$ in an open neighborhood of $\theta_0$ , i.e., $$\nabla^2_{\theta}\int_{\mathcal{X}} p(x; \theta) dx = \int_{\mathcal{X}} \nabla^2_{\theta}p(x; \theta) dx$$ for all $\theta \in \Theta^*$. Note that this implies first-order partial derivatives can be passed under the integral sign as well.
