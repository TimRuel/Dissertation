...
$$\mathcal{S}(\theta) = \mathcal{S}(\theta; \mathbf{x}_n) = \nabla\ell(\theta; \mathbf{x}_n) = \begin{pmatrix} \frac{\partial \ell}{\partial\theta_1} \\ \vdots \\ \frac{\partial \ell}{\partial\theta_d}  \end{pmatrix}, \> \> \theta \in \Theta.$${#eq-score} 
...
$$\mathcal{I}(\theta) = \mathcal{I}(\theta; \mathbf{x}_n) = -\nabla^2\ell(\theta; \mathbf{x}_n) = -
\begin{pmatrix} 
...
\begin{enumerate}[label = RC\arabic*)]
  ...
  \item $\mathcal{P}$ is identifiable, i.e., $P_{\theta_1} = P_{\theta_2} \implies \theta_1 = \theta_2$ for all $\theta_1, \theta_2 \in \Theta$.
  \item  The distributions in $\mathcal{P}$ have a common support $\mathcal{X}$ not depending on $\theta$.
  \item The parameter space $\Theta$ is a convex set.
  \item The Fisher information matrix $\mathscr{I}(\theta)$ is positive definite and has only finite components for all $\theta \in \Theta$.
  \item There exists an open set $\Theta^* \subseteq \Theta$ of which $\theta_0$ is an interior point.
  \item For almost all $x$ and for all $\theta \in \Theta$, $p(x; \theta)$ is twice continuously differentiable with respect to $\theta$.
  \item The integral $\int_{-\infty}^{\infty} p(x; \theta) dx$ can be differentiated twice under the integral sign with respect to $\theta \in \Theta$.
  \item There exists a random function $M(x)$ (that does not depend on $\theta$) satisfying $E_{\theta_0}[M(X)] < \infty$ and a radius $r > 0$ such that for all $\theta \in N_r(\theta_0)$ and all integers $1 \leq i, j, k \leq d$, $|\nabla^3\ell(\theta;x)_{ijk}| \leq M(x)$.
\end{enumerate}

The phrase "almost all $x$" in RC6) means that there may exist a set of points $\mathcal{X}$ satisfying $\int_{\mathcal{X}} p_{\theta}(x) dx = 0$ for which $p(x; \theta)$ is not differentiable with respect to $\theta$ at any point $x \in \mathcal{X}$. 
...
\text{E}_{\theta}\Bigg[\frac{\partial^2}{\partial \theta^2} \ell(\theta; X)\Bigg] + \text{E}_{\theta}\Bigg[\Bigg(\frac{\partial}{\partial \theta} \ell(\theta; X)\Bigg)^2\Bigg] &= \text{E}_{\theta}\Bigg[\frac{\frac{\partial^2}{\partial \theta^2} p(X; \theta)}{p(X; \theta)}\Bigg] \\
            &= \int_{\mathbbm{R}^n} \Bigg[\frac{\frac{\partial^2}{\partial \theta^2} p(x; \theta)}{p(x; \theta)}\Bigg] p(x; \theta) d\mathbf{x}_n \\
            &= \int_{\mathbbm{R}^n} \frac{\partial^2}{\partial \theta^2} p(x; \theta) dx \\
            &= \frac{d^2}{d \theta^2} \int_{\mathbbm{R}^n} p(x; \theta) dx \\
            &= \frac{d^2}{d \theta^2} 1 \\
            &= 0. 
\end{aligned}
$$
Therefore, $$\text{E}_{\theta}\Bigg[\frac{\partial^2}{\partial \theta^2} \ell(\theta; X)\Bigg] + \text{E}_{\theta}\Bigg[\Bigg(\frac{\partial}{\partial \theta} \ell(\theta; X)\Bigg)^2 \Bigg] = 0.$${#eq-BI2}

@eq-BI2 is called the second Bartlett identity. It follows that, for models satisfying RC1-3) the expected information can be rewritten as $$\text{E}_{\theta}\Bigg[-\frac{\partial^2}{\partial \theta^2} \ell(\theta; X)\Bigg] = \mathbbm{V}[\ell_{\theta}(\theta; x); \theta].$${#eq-BI3} 
...
To do this, first note that the likelihood function for $\theta$ based on $\mathbf{x}_n = (x_1, ..., x_n)$ as $$L(\theta; \mathbf{x}_n) = p_{\theta}(\mathbf{x}_n) = \prod_{i=1}^n p_{\theta}(x_i).$$ We may therefore write the log-likelihood function as 
$$
\begin{aligned}
\ell(\theta; \mathbf{x}_n) &= \log L(\theta; \mathbf{x}_n) \\
                         &= \log p_{\theta}(\mathbf{x}_n) \\
                         &= \log\Bigg[ \prod_{i=1}^n p_{\theta}(x_i)\Bigg] \\
...
@eq-score_additive implies that a Taylor series expansion of $\mathcal{S}(\theta; \mathbf{x}_n)$ will be equal to the sum of the Taylor series expansions of its individual contributions, plus a remainder term that grows with $n$. We have assumed each $x_i$ is identically distributed, so it suffices to consider the expansion for an arbitrary contribution, $\mathcal{S}(\theta; x_i)$.

Since $\theta$ is a $d \times 1$ vector, the $k$-th derivative of the log-likelihood with respect to $\theta$ will be a $k$-dimensional array having $d$ entries along each of its $k$ indices. In particular, the score function (i.e. the first derivative of the log-likelihood) will also be a $d \times 1$ vector, $$\mathcal{S}(\theta; x_i) = \begin{pmatrix} \mathcal{S}_1(\theta; x_i) \\ \vdots \\ \mathcal{S}_d(\theta; x_i) \end{pmatrix},$$ where each component is a function $S_j: \Theta \to \mathbbm{R}$. Similarly, the first and second derivatives of the score function will be a $d \times d$ matrix and a three-dimensional $d \times d \times d$ array, respectively. To simplify notation, we will perform a component-wise first-order Taylor series expansion of the score function around the point $\theta = \theta_0$ wherein each component $\mathcal{S}_j(\theta; x_i)$ of $\mathcal{S}(\theta; x_i)$ is expanded separately.
...
\mathcal{S}_j(\theta; \mathbf{x}_n) &= \sum_{i=1}^n \mathcal{S}_j(\theta; x_i) \\
                                  ...
                                  &= \mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg[\nabla \mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg\{\sum_{i=1}^nM(x_i) \Bigg\}O(||\theta - \theta_0||)\Bigg]^T(\theta - \theta_0) \\
                                  &= \mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg[\frac{1}{n}\nabla \mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg\{\frac{1}{n}\sum_{i=1}^nM(x_i) \Bigg\}O(||\theta - \theta_0||)\Bigg]^Tn(\theta - \theta_0).
\end{aligned} 
$$
If we divide through by $\sqrt n$, we arrive at $$\frac{1}{\sqrt n}\mathcal{S}_j(\theta; \mathbf{x}_n) = \frac{1}{\sqrt n}\mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg[\frac{1}{n}\nabla \mathcal{S}_j(\theta_0; \mathbf{x}_n) + \Bigg\{\frac{1}{n}\sum_{i=1}^nM(x_i) \Bigg\}O(||\theta - \theta_0||)\Bigg]^T\sqrt{n}(\theta - \theta_0).$$
