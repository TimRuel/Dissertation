...
We can then write the integrated likelihood for $\psi$ in terms of $\overset{\sim}{L}(u; \psi)$ as follows: $$\bar{L}(\psi) = \int_{\mathcal{U}} \tilde{L}(u; \psi)\tilde{\pi}(u)du,$${#eq-ZSE_IL10} where $\tilde{\pi}(u)$ is a density on $\mathcal{U}$ of our choosing. If $\tilde{\pi}(u)$ doesn't depend on $\psi$, the integrated likelihood given by @eq-ZSE_IL10 will have the properties we desire.

Suppose $\mathcal{U} = \Theta$ so that $$\bar{L}(\psi) = \int_{\Theta} \tilde{L}(u; \psi)\tilde{\pi}(u)du.$$ We can further rewrite this as $$\bar{L}(\psi) = \int_{\Theta} \frac{\tilde{L}(u; \psi)}{L(u)} L(u)\tilde{\pi}(u)du.$${#eq-ZSE_IL11} From a Bayesian point of view, the quantity $L(u)\tilde{\pi}(u)$ can be thought of as a posterior density for the "parameter" $u$ up to some proportionality constant. When $\tilde{\pi}(u)$ is a conjugate prior for $L(u)$ such that the posterior density $L(u)\tilde{\pi}(u)$ has a closed form solution equal to a known distribution, then random samples can be drawn directly from the posterior using modern statistical computing packages. If no such closed form solution exists, then random samples can still obtained through Monte Carlo methods such as importance sampling or MCMC.  

In either case, once random variates $u_1, ..., u_R$ have been sampled from $L(u)\tilde{\pi}(u)$, a simple empirical estimate to $\bar{L}(\psi)$ at a particular value of $\psi$ is given by $$\hat{\bar{L}}(\psi) = \frac{1}{R} \sum_{i = 1}^R \frac{\tilde{L}(u_i; \psi)}{L(u_i)}.$${#eq-ZSE_IL12} Repeating this procedure for every value of $\psi \in \Psi$ will give an overall shape for $\hat{\bar{L}}(\psi)$, which can be used to conduct inference for $\psi_0$ without interference from a nuisance parameter. 
