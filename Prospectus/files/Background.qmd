...
This definition be extended to include the more common scenario in which the sample consists of multiple observations. For a sample of size $n$ taken from our sample space $\mathcal{S}$, the likelihood is defined as $$L(\theta) \equiv L(\theta;x_1, ..., x_n) = p(x_1, ..., x_n; \theta), \> \> \theta \in \Theta.$${#eq-likelihood_multi} That is, it is equal to the joint probability of the observations $x_1, ..., x_n$, considered as a function of $\theta$.
