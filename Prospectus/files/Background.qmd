...
\subsection{Model Parameter Decomposition}
  ...
  It is often the case that we are not interested in estimating the full parameter $\theta \in \Theta$, but rather a sub-parameter $\psi$ taking values in a set $\Psi$, where $|\Psi| < |\Theta|$. In such an event, we refer to $\psi$ as the *parameter of interest*. 

  Consider first the case in which $\Psi \subset \Theta$. That is, all components of $\psi$ are also components of $\theta$. 
