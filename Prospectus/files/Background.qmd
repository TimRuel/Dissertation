...
If $\mathcal{S}$ is uncountable, then the random variable $X$ generating the outcomes we observe from it must instead be continuous.For the purposes of my research, the only continuous random variables I will consider are those that admit a probability density function. Letting $p(x;\theta)$ denote this density function for $X$, 
