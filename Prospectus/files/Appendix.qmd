...
The *neighborhood* of radius $r$ at a point $\mathbf{p} \in \mathbb{R}^d$ is the set of all points $\mathbf{x} \in \mathbb{R}^d$ such that the distance between $\mathbf{p}$ and $\mathbf{x}$ is less than $r$. We use the following notation to refer to such neighborhoods: $$N_r(\mathbf{p}) = \{\mathbf{x}: ||\mathbf{x} - \mathbf{p}|| < r \}.$$
