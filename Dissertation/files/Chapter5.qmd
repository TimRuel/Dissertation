...
\end{aligned}
$${#eq-1}
where $t_j' = \frac{t_j}{\sum_{j=1}^J t_j}$ and $\hat{\theta}_j = Y_j / t_j$ is the observed rate of infection at hospital $j$. If $\theta_j$ represents its associated estimand, we can similarly rewrite our expression for $\psi$ in a similar form as $$\psi = \sum_{j=1}^J t_j' \theta_j.$${#eq-2}
...
\subsection{Poisson regression model with fixed effects}
...
To estimate the value of the ZSE-parameterized integrated likelihood at a specific value of $\psi$, say $\psi_1$, using the method in Chapter 3, we use the following procedure.
...
\subsection{Poisson regression model with mixed effects}
...
\section{Estimating weighted sums of Poisson rates}
...
\section{Detecting overdispersion}
...
\section{Detecting zero-inflation}
