...
When $X$ is discrete, we may interpret $L(\symbf{\theta}; x)$ as the probability that $X = x$ given that $\symbf{\theta}$ is the true parameter value.^[Whichever value of $\symbf{\theta}$ we choose to plug into $L(\symbf{\theta}; x)$ is the value that we are currently "pretending" is the true one, regardless of whether or not it actually equals $\symbf{\theta}_0$ in reality.] Crucially, this is *not* equivalent to the inverse probability that $\symbf{\theta}$ is the true parameter value given $X = x$. The likelihood does not directly tell us anything about the probability that $\symbf{\theta}$ assumes any particular value at all. Though intuitively appealing, this interpretation constitutes a fundamental misunderstanding of what a likelihood function is, and great care must be taken to avoid it. 
...
$$\mathcal{I}_{\mathbf{X}_n}(\symbf{\theta}) = -\mathbf{H}_{\symbf{\theta}}\Big(\ell(\symbf{\theta}; \mathbf{x}_n)\Big) = -\mathbf{J}_{\symbf{\theta}}\Big(\mathcal{S}(\symbf{\theta}; \mathbf{x}_n)\Big)^\top = -
\begin{pmatrix} 
...
This can be extended to include as many candidate parameter values as we would like. For $n$ potential estimates of $\symbf{\theta}_0$, the best is the one that corresponds to the highest value of the likelihood function. Following this line of reasoning to its natural conclusion, a sensible choice for an estimate of $\symbf{\theta}_0$ is any value that maximizes the likelihood function based on an observed dataset $\mathbf{x}_n$. Hence, let $\hat{\symbf{\theta}}\in \Theta$ be any parameter value that renders the likelihood at the observed $\mathbf{X}_n = \mathbf{x}_n$ as large as possible, i.e., $$L(\hat{\symbf{\theta}}; \mathbf{x}_n) = \sup_{\symbf{\theta} \in \Theta} L(\symbf{\theta}; \mathbf{x}_n).$${#eq-MLE8} We call such a value a *maximum likelihood estimate* of $\symbf{\theta}_0$. This definition of $\hat{\symbf{\theta}}$ as a maximizer of $L(\hat{\symbf{\theta}}; \mathbf{x}_n)$ necessarily makes it a function of the observed data. When this function is measurable, then we can further define the *maximum likelihood estimator* (MLE) of $\symbf{\theta}_0$ as the statistic $\hat{\symbf{\theta}}(\mathbf{X}_n)$ for which we observe the value $\hat{\symbf{\theta}}(\mathbf{x}_n)$. 
...
$$\mathscr{I}_{X}(\theta) = \text{Var}_{\theta}\Bigg[\frac{\partial}{\partial \theta} \ell(\theta; X)\Bigg] = \text{E}_{\theta}\Bigg[\bigg (\frac{\partial}{\partial \theta}\ell(\theta; X) \bigg)^2\Bigg] = \text{E}_{\theta}\Bigg[-\frac{\partial^2}{\partial \theta^2} \ell(\theta; X)\Bigg] = \text{E}_{\theta}\big[\mathcal{I}_X(\theta)\big].
$${#eq-BI4} 
...
\section{Laplace's Method}\label{appendix:laplace}

Laplace's method approximates integrals of the form
$$
I = \int_{\mathbbm{R}^d} \exp\{f(\mathbf{u})\}\, d\mathbf{u}
$$
where $f : \mathbbm{R}^d \to \mathbbm{R}$ is smooth and has a unique global maximum at $\hat{\mathbf{u}}$. Expanding $f$ to second order around $\hat{\mathbf{u}}$, we have
$$
\begin{aligned}
f(\mathbf{u}) &\approx f(\hat{\mathbf{u}}) + (\mathbf{u} - \hat{\mathbf{u}})^\top\nabla f(\hat{\mathbf{u}}) + \tfrac{1}{2}(\mathbf{u} - \hat{\mathbf{u}})^\top \nabla^2 f(\hat{\mathbf{u}}) (\mathbf{u} - \hat{\mathbf{u}}) \\ &= f(\hat{\mathbf{u}}) + \tfrac{1}{2}(\mathbf{u} - \hat{\mathbf{u}})^\top \nabla^2 f(\hat{\mathbf{u}}) (\mathbf{u} - \hat{\mathbf{u}}),
\end{aligned}
$$
...
$$
I \approx (2\pi)^{d/2} \lvert {-\nabla^2 f(\hat{\mathbf{u}})} \rvert^{-1/2} \exp\{f(\hat{\mathbf{u}})\},
$$
so that
$$
\log I \approx f(\hat{\mathbf{u}}) + \tfrac{d}{2}\log(2\pi) - \tfrac{1}{2}\log\lvert {-\nabla^2 f(\hat{\mathbf{u}})} \rvert.
$${#eq-laplace-general}
The approximation is exact when $f$ is quadratic, and has relative error $O(n^{-1})$ in general, where $n$ is the effective sample size driving the concentration of $\exp\{f\}$ around $\hat{\mathbf{u}}$. @Tierney1986 showed that for integrals arising in Bayesian computation --- specifically, ratios of the form
$$
...
\subsection{Application to the Random Effects Multinomial Model}\label{appendix:laplace-re}

For notational compactness, let $\textbf{C} = [\textbf{A} \mid \textbf{B}] \in \mathbbm{R}^{p \times (J-1)}$ denote the full coefficient matrix and let $\tilde{\symbf{x}}_{it} = (1, \symbf{x}_{it}^\top)^\top$ denote the design vector with intercept prepended, so that the linear predictor for observation $(i,t)$ is $\textbf{C}^\top \tilde{\symbf{x}}_{it} + \symbf{u}_i$. The cluster-level integral from @eq-cluster-likelihood has the form $\int \exp\{g_i(\mathbf{u})\}\,d\mathbf{u}$, where
$$
g_i(\mathbf{u}) = \sum_{t=1}^{m_i} \sum_{j=1}^{J} y_{itj} \log \theta_j(\tilde{\symbf{x}}_{it}; \textbf{C}, \mathbf{u}) - \tfrac{1}{2} \mathbf{u}^\top \symbf{\Sigma}^{-1} \mathbf{u},
$${#eq-laplace-gi}
obtained by writing $\phi(\mathbf{u}_i; \mathbf{0}, \symbf{\Sigma}) = (2\pi)^{-(J-1)/2} |\symbf{\Sigma}|^{-1/2} \exp\{-\tfrac{1}{2}\mathbf{u}_i^\top \symbf{\Sigma}^{-1} \mathbf{u}_i\}$ and absorbing the normalizing constants. Note that $g_i(\mathbf{u})$ is precisely the complete-data log-likelihood $\ell_{\mathrm{c}}$ from @eq-complete-data-loglik evaluated at cluster $i$, minus the log-prior on $\mathbf{u}_i$. The Laplace approximation therefore approximates the log of the marginal likelihood — the log-outside-the-integral quantity from @eq-random-effects-loglikelihood — by evaluating the log-inside-the-integral quantity $g_i$ at its mode and correcting for curvature. The mode $\hat{\mathbf{u}}_i = \operatorname{argmax}_{\mathbf{u}}\, g_i(\mathbf{u})$ is the posterior mode of $\mathbf{u}_i$ given cluster $i$'s observations and $\textbf{C}$. Differentiating @eq-laplace-gi twice with respect to $\mathbf{u}$ and evaluating at $\hat{\mathbf{u}}_i$ gives the negative Hessian
$$
-\nabla^2 g_i(\hat{\mathbf{u}}_i) = \sum_{t=1}^{m_i} \mathbf{W}_t(\hat{\mathbf{u}}_i) + \symbf{\Sigma}^{-1},
$${#eq-laplace-hessian}
where
$$
\mathbf{W}_t = \operatorname{diag}(\symbf{\pi}_t) - \symbf{\pi}_t \symbf{\pi}_t^\top, \quad \symbf{\pi}_t = \bigl(\theta_2(\tilde{\symbf{x}}_{it}; \textbf{C}, \hat{\mathbf{u}}_i), \dots, \theta_J(\tilde{\symbf{x}}_{it}; \textbf{C}, \hat{\mathbf{u}}_i)\bigr)^\top
$$
is the multinomial covariance matrix of the non-baseline fitted probabilities at the posterior mode. The term $\symbf{\Sigma}^{-1}$ reflects the curvature of the random effects prior and $\sum_t \mathbf{W}_t$ reflects the curvature of the data likelihood; together they determine the sharpness of the posterior around $\hat{\mathbf{u}}_i$. Applying @eq-laplace-general with $d = J - 1$ and canceling the $(2\pi)^{(J-1)/2}$ factors from the normalizing constant of $\phi$ yields
$$
\log L_i(\textbf{C}, \symbf{\Sigma}) \approx g_i(\hat{\mathbf{u}}_i) - \tfrac{1}{2}\log|\symbf{\Sigma}| - \tfrac{1}{2}\log\Bigl\lvert \textstyle\sum_{t} \mathbf{W}_t(\hat{\mathbf{u}}_i) + \symbf{\Sigma}^{-1} \Bigr\rvert.
$${#eq-laplace-approx}

When $\textbf{C}$ is treated as variable rather than fixed, the gradient of $\log I$ with respect to $\textbf{C}$ requires accounting for the dependence of $\hat{\mathbf{u}}_i(\textbf{C})$ on $\textbf{C}$ through the implicit equation $\nabla_{\mathbf{u}} g_i(\hat{\mathbf{u}}_i; \textbf{C}) = \mathbf{0}$. By the implicit function theorem,
$$
\frac{d\hat{\mathbf{u}}_i}{d\textbf{C}} = -\bigl[\nabla^2_{\mathbf{u}} g_i(\hat{\mathbf{u}}_i)\bigr]^{-1} \nabla_{\textbf{C}} \nabla_{\mathbf{u}} g_i(\hat{\mathbf{u}}_i).
$$
The standard approximation from @breslow1993 sets $d\hat{\mathbf{u}}_i / d\textbf{C} = 0$, treating the posterior mode as fixed when differentiating with respect to $\textbf{C}$. Under this approximation the gradient of $\ell^{\mathrm{LA}}$ with respect to $\textbf{C}$ reduces to
$$
\nabla_{\textbf{C}} \ell^{\mathrm{LA}}(\textbf{C}, \symbf{\Sigma}) \approx \sum_{i=1}^n \sum_{t=1}^{m_i} \tilde{\symbf{x}}_{it} \bigl(\mathbf{Y}_{it} - \symbf{\pi}_{it}\bigr)^\top,
$${#eq-laplace-grad}
where $\mathbf{Y}_{it} \in \{0,1\}^{J-1}$ is the response indicator vector with the baseline category dropped and $\symbf{\pi}_{it} = \symbf{\pi}_t(\hat{\mathbf{u}}_i)$ is the corresponding vector of non-baseline fitted probabilities at the posterior mode. Gradients with respect to $\textbf{A}$ and $\textbf{B}$ are the corresponding row submatrices of $\nabla_{\textbf{C}} \ell^{\mathrm{LA}}$. The expression in @eq-laplace-grad has the same form as the score of an ordinary multinomial logistic regression, with $\hat{\mathbf{u}}_i$ acting as a cluster-level offset in the linear predictor. The approximation error is $O(m_i^{-1})$ per cluster, matching the order of the Laplace approximation itself.

In the context of this paper, @eq-laplace-grad is used in two places: in estimation of $\textbf{C}$, where it drives the optimization of $\ell^{\mathrm{LA}}$ with respect to the fixed effects, and in the computation of profile and integrated likelihoods, where it is supplied to the constrained optimizer at each point of the $\psi$ grid. In the former case an inexact gradient affects the path to convergence but not the solution itself, since convergence is assessed by the log-likelihood value. In both cases the approximation is unlikely to be consequential for the designs considered in the simulation study.

\section{Properties of the Integrated Likelihood}

\subsection{Asymptotic Theory}

\subsubsection{One-Index Asymptotics}
...
\subsubsection{Two-Index Asymptotics}
...
Since the observations between the strata are independent, we may write the likelihood and log-likelihood functions for the entire model as $$L(\psi, \lambda) = \prod_{i=1}^q L^{(i)}(\psi, \lambda_i)$${#eq-2idx_L} and $$\ell(\psi, \lambda) = \sum_{i=1}^q \ell^{(i)}(\psi, \lambda_i),$${#eq-2idx_l} respectively. If we define the weight function $$G(\lambda; \psi) = \prod_{i=1}^qg(\lambda_i; \psi)$${#eq-2idx_weight} then the integrated likelihood function for $\psi$ becomes separable. That is,
$$
\begin{aligned}
