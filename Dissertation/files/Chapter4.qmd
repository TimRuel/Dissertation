...
Another important property of $D(\symbf{\theta})$ is its invariance under permutations of the category labels. Let $S_J$ denote the symmetric group of all permutations of $\{1, \dots, J\}$. Then for any permutation $\pi \in S_J$,
$$
D(\theta_{\pi(1)}, \dots, \theta_{\pi(J)}) = D(\theta_1, \dots, \theta_J),
$$
since the sum of squares is unaffected by the ordering of its terms. This reflects the fact that Simpson's index is a measure of concentration of probability mass, without regard to which specific categories carry that mass. Consequently, $\Omega_{\hat{\psi}}$ is closed under any action of $S_J$: if $\symbf{\omega} \in \Omega_{\hat{\psi}}$, then so is every permutation of its components. This symmetry has direct implications for how elements must be sampled from $\Omega_{\hat{\psi}}$. See Section \ref{sec-simpson-sampling-strategy} for further details.

\subsection{Simple logit model}

\subsubsection{$\Omega_{\hat{\psi}}$ Sampling Strategy}
\label{sec-simpson-sampling-strategy}
