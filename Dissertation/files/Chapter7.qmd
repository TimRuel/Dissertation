...
Now suppose there is a set of covariates that we have reason to believe is influencing the counts we record from the Poisson process in each group. For example, that at the same time we record Poisson counts from each group, we also make observations from a set of covariates we have reason to believe are influencing the 
