...
Let $\Theta \subseteq \mathbb{R}^n_+$ represent the space of possible values for $\theta$, and suppose we have a real-valued parameter of interest $\psi = \tau(\theta)$, where $\tau: \Theta \to \Psi$ is a known twice continuously differentiable function. 
