...
for this excess is to model the data using a mixture of two
distributions. The first is a simple Bernoulli process that generates
zeros and the other is one that generates counts, some of which may also
be zeros. When this count-generating process is governed by a Poisson
distribution, we call the resulting mixture a zero-inflated Poisson
(ZIP) model.

Let \(X \sim \text{Poisson}(\mu)\) and
