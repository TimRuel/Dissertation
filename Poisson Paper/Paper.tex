...
Consider the task of estimating the weighted sum of a set of Poisson
means corresponding to \(n\) distinct populations, where \(n\) is a
known positive integer. Let \(\theta_i\) denote the mean of population
\(i\), and suppose we have drawn an independent sample of size \(m_i\)
from each population, where \(X_{ij}\) represents the \(j\)th count from
the \(i\)th sample. Then we have
...
\(w_i\) is a known constant greater than zero.
